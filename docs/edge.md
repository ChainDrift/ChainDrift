...
!!! Danger "Deprecated functionality"
    ...
    It will currently not receive new features until either someone stepped forward to take up ownership of that module - or we'll decide to remove edge from chaindrift.
...
```
usage: chaindrift edge [-h] [-v] [--logfile FILE] [-V] [-c PATH] [-d PATH]
...
```bash
chaindrift edge --stoplosses=-0.01,-0.1,-0.001 #min,max,step
```

### Advanced use of timerange

```bash
chaindrift edge --timerange=20181110-20181113
```
