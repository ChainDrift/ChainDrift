...
To download data (candles / OHLCV) needed for backtesting and hyperoptimization use the `chaindrift download-data` command.

If no additional parameter is specified, chaindrift will download data for `"1m"` and `"5m"` timeframes for the last 30 days.
...
!!! Tip "Tip: Updating existing data"
    If you already have backtesting data available in your data-directory and would like to refresh this data up to today, chaindrift will automatically calculate the missing timerange for the existing pairs and the download will occur from the latest available point until "now", neither `--days` or `--timerange` parameters are required. Chaindrift will keep the available data and only download the missing data.  
...
```
usage: chaindrift download-data [-h] [-v] [--logfile FILE] [-V] [-c PATH]
                               [-d PATH] [--userdir PATH]
...
!!! Tip "Downloading all data for one quote currency"
    ...
    `chaindrift download-data --exchange binance --pairs .*/USDT <...>`. The provided "pairs" string will be expanded to contain all active pairs on the exchange.
...
```bash
chaindrift download-data --exchange binance
```
...
```bash
chaindrift download-data --exchange binance --pairs ETH/USDT XRP/USDT BTC/USDT
```

or as regex (in this case, to download all active USDT pairs)

```bash
chaindrift download-data --exchange binance --pairs .*/USDT
```
...
``` bash
chaindrift download-data --exchange binance --pairs ETH/USDT XRP/USDT BTC/USDT --prepend --timerange 20210101-20220101
```

!!! Note
    Chaindrift will ignore the end-date in this mode if data is available, updating the end-date to the existing data start point.

### Data format

Chaindrift currently supports the following data-formats:
...
``` bash
time chaindrift list-data --show-timerange --data-format-ohlcv <dataformat>
```
...
```
usage: chaindrift convert-data [-h] [-v] [--logfile FILE] [-V] [-c PATH]
                              [-d PATH] [--userdir PATH]
...
The following command will convert all candle (OHLCV) data available in `~/.chaindrift/data/binance` from json to jsongz, saving diskspace in the process.
It'll also remove original json data files (`--erase` parameter).

``` bash
chaindrift convert-data --format-from json --format-to jsongz --datadir ~/.chaindrift/data/binance -t 5m 15m --erase
```

## Sub-command convert trade data

```
usage: chaindrift convert-trade-data [-h] [-v] [--logfile FILE] [-V] [-c PATH]
                                    [-d PATH] [--userdir PATH]
                                    [-p PAIRS [PAIRS ...]] --format-from
...
The following command will convert all available trade-data in `~/.chaindrift/data/kraken` from jsongz to json.
It'll also remove original jsongz data files (`--erase` parameter).

``` bash
chaindrift convert-trade-data --format-from jsongz --format-to json --datadir ~/.chaindrift/data/kraken --erase
```
...
```
usage: chaindrift trades-to-ohlcv [-h] [-v] [--logfile FILE] [-V] [-c PATH]
                                 [-d PATH] [--userdir PATH]
                                 [-p PAIRS [PAIRS ...]]
                                 [-t TIMEFRAMES [TIMEFRAMES ...]]
                                 [--exchange EXCHANGE]
...
``` bash
chaindrift trades-to-ohlcv --exchange kraken -t 5m 1h 1d --pairs BTC/EUR ETH/EUR
```
...
```
usage: chaindrift list-data [-h] [-v] [--logfile FILE] [-V] [-c PATH] [-d PATH]
...
```bash
> chaindrift list-data --userdir ~/.chaindrift/user_data/
...
``` bash
> chaindrift list-data --show --trades
...
```bash
chaindrift download-data --exchange kraken --pairs XRP/EUR ETH/EUR --days 20 --dl-trades
```
