...
ChainUI now has it's own dedicated [documentation section](freq-ui.md) - please refer to that section for all information regarding the ChainUI.
...
You can consume the API by using `chaindrift-client` (also available as `scripts/rest_client.py`).
This command can be installed independent of the bot by using `pip install chaindrift-client`.

This module is designed to be lightweight, and only depends on the `requests` and `python-rapidjson` modules, skipping all heavy dependencies chaindrift otherwise needs.

``` bash
chaindrift-client <command> [optional parameters]
```
...
``` bash
chaindrift-client --config rest_config.json <command> [optional parameters]
```

Commands with many arguments may require keyword arguments (for clarity) - which can be provided as follows:

``` bash
chaindrift-client --config rest_config.json forceenter BTC/USDT long enter_tag=GutFeeling
```

This method will work for all arguments - check the "show" command for a list of available parameters.

??? Note "Programmatic use"
    The `chaindrift-client` package (installable independent of chaindrift) can be used in your own scripts to interact with the chaindrift API.
    to do so, please use the following:

    ``` python
    from chaindrift_client import FtRestClient
...
The API Server includes a websocket endpoint for subscribing to RPC messages from the chaindrift Bot.
...
This is also used to setup [Producer/Consumer mode](producer-consumer.md) in Chaindrift.
...
For a list of message types, please refer to the RPCMessageType enum in `chaindrift/enums/rpcmessagetype.py`
...
Please make sure to replace `<chaindrift_listen_ip>` (and the subsequent port) with the IP and Port matching your configuration/setup.
...
http {
    ...
    server {
        ...
        location / {
            ...
            proxy_pass http://<chaindrift_listen_ip>:8080;
...
!!! Tip "SSL certificates"
    ...
    While this will protect your data in transit, we do not recommend to run the chaindrift API outside of your private network (VPN, SSH tunnel).
...
!!! Note
    The below should be done in an application (a Chaindrift REST API client, which fetches info via API), and is not intended to be used on a regular basis.

Chaindrift's REST API also offers JWT (JSON Web Tokens).
You can login using the following command, and subsequently use the resulting access_token.

``` bash
> curl -X POST --user Chaindriftr http://localhost:8080/api/v1/token/login
