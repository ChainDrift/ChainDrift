...
A sample of this can be found below, which is identical to the Default Hyperopt loss implementation. A full sample can be found in [userdata/hyperopts](https://github.com/chaindrift/chaindrift/blob/develop/chaindrift/templates/sample_hyperopt_loss.py).
...
from chaindrift.optimize.hyperopt import IHyperOptLoss
...
class SuperDuperHyperOptLoss(IHyperOptLoss):
    ...
    def hyperopt_loss_function(
        ...
        **kwargs,
        ...
        This is the legacy algorithm (used until now in chaindrift).
...
```python
from chaindrift.optimize.space import Categorical, Dimension, Integer, SKDecimal
...
For the additional spaces, scikit-optimize (in combination with Chaindrift) provides the following space types:
...
* `SKDecimal` - Pick from a range of decimal numbers with limited precision (e.g. `SKDecimal(0.1, 0.5, decimals=3, name='adx')`). *Available only with chaindrift*.
* `Real` - Pick from a range of decimal numbers with full precision (e.g. `Real(0.1, 0.5, name='adx')`

You can import all of these from `chaindrift.optimize.space`, although `Categorical`, `Integer` and `Real` are only aliases for their corresponding scikit-optimize Spaces. `SKDecimal` is provided by chaindrift for faster optimizations.

``` python
from chaindrift.optimize.space import Categorical, Dimension, Integer, SKDecimal, Real  # noqa
```
