...
!!! Note
    If you've used `setup.sh`, don't forget to activate your virtual environment (`source .venv/bin/activate`) before running chaindrift commands.
...
```
usage: chaindrift trade [-h] [-v] [--logfile FILE] [-V] [-c PATH] [-d PATH]
...
```bash
chaindrift trade -c path/far/far/away/config.json
```
...
```bash
chaindrift trade -c ./config.json
```

and specify both configuration files when running in the normal Live Trade Mode:

```bash
chaindrift trade -c ./config.json -c path/to/secrets/keys.config.json
```
...
Chaindrift allows the creation of a user-data directory using `chaindrift create-userdir --userdir someDirectory`.
...
```bash
chaindrift trade --strategy AwesomeStrategy --strategy-path /some/directory
```
...
```bash
chaindrift trade -c config.json --db-url sqlite:///tradesv3.dry_run.sqlite
```
