...
This page provides you some basic concepts on how Chaindrift works and operates.

## Chaindrift terminology
...
All profit calculations of Chaindrift include fees. For Backtesting / Hyperopt / Dry-run modes, the exchange default fee is used (lowest tier on the exchange). For live operations, fees are used as applied by the exchange (this includes BNB rebates etc.).

## Pair naming

Chaindrift follows the [ccxt naming convention](https://docs.ccxt.com/#/README?id=consistency-of-base-and-quote-currencies) for currencies.
...
Starting chaindrift in dry-run or live mode (using `chaindrift trade`) will start the bot and start the bot iteration loop.
