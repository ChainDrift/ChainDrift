...
Creates the directory structure to hold your files for chaindrift.
...
```
usage: chaindrift create-userdir [-h] [--userdir PATH] [--reset]
...
```
usage: chaindrift new-config [-h] [-c PATH]
...
!!! Warning
    Only vital questions are asked. Chaindrift offers a lot more configuration possibilities, which are listed in the [Configuration documentation](configuration.md#configuration-parameters)

### Create config examples

```
$ chaindrift new-config --config user_data/config_binance.json
...
```
usage: chaindrift show-config [-h] [--userdir PATH] [-c PATH]
                             [--show-sensitive]
...
``` output
usage: chaindrift new-strategy [-h] [--userdir PATH] [-s NAME]
...
```bash
chaindrift new-strategy --userdir ~/.chaindrift/ --strategy AwesomeStrategy
```

Using the advanced template (populates all optional functions and methods)

```bash
chaindrift new-strategy --strategy AwesomeStrategy --template advanced
```
...
```
usage: chaindrift list-strategies [-h] [-v] [--logfile FILE] [-V] [-c PATH]
                                 [-d PATH] [--userdir PATH]
...
``` bash
chaindrift list-strategies
```

Example: Search strategies  directory within the userdir.

``` bash
chaindrift list-strategies --userdir ~/.chaindrift/
```

Example: Search dedicated strategy path.

``` bash
chaindrift list-strategies --strategy-path ~/.chaindrift/strategies/
```
...
```
usage: chaindrift list-hyperoptloss [-h] [-v] [--logfile FILE] [-V] [-c PATH]
                                   [-d PATH] [--userdir PATH]
...
Use the `list-chainaimodels` subcommand to see all chainAI models available.

This subcommand is useful for finding problems in your environment with loading chainAI models: modules with models that contain errors and failed to load are printed in red (LOAD FAILED), while models with duplicate names are printed in yellow (DUPLICATE NAME).

```
usage: chaindrift list-chainaimodels [-h] [-v] [--logfile FILE] [-V] [-c PATH]
                                   [-d PATH] [--userdir PATH]
                                   [--chainaimodel-path PATH] [-1] [--no-color]

optional arguments:
  -h, --help            show this help message and exit
  --chainaimodel-path PATH
                        Specify additional lookup path for chainaimodels.
  -1, --one-column      Print output in one column.
  --no-color            Disable colorization of hyperopt results. May be
                        useful if you are redirecting output to a file.
...
```
usage: chaindrift list-exchanges [-h] [-1] [-a]
...
```
$ chaindrift list-exchanges
Exchanges available for Chaindrift:
...
Example: see all exchanges supported by the ccxt library (including 'bad' ones, i.e. those that are known to not work with Chaindrift)

```
$ chaindrift list-exchanges -a
...
```
usage: chaindrift list-timeframes [-h] [-v] [--logfile FILE] [-V] [-c PATH]
                                 [-d PATH] [--userdir PATH]
                                 [--exchange EXCHANGE] [-1]
...
```
$ chaindrift list-timeframes -c config_binance.json
...
Timeframes available for the exchange `binance`: 1m, 3m, 5m, 15m, 30m, 1h, 2h, 4h, 6h, 8h, 12h, 1d, 3d, 1w, 1M
```

* Example: enumerate exchanges available for Chaindrift and print timeframes supported by each of them:
```
$ for i in `chaindrift list-exchanges -1`; do chaindrift list-timeframes --exchange $i; done
```
...
For pairs traded by Chaindrift the pair quote currency is defined by the value of the `stake_currency` configuration setting.
...
```
usage: chaindrift list-markets [-h] [-v] [--logfile FILE] [-V] [-c PATH]
                              [-d PATH] [--userdir PATH] [--exchange EXCHANGE]
                              [--print-list] [--print-json] [-1] [--print-csv]
                              [--base BASE_CURRENCY [BASE_CURRENCY ...]]
                              [--quote QUOTE_CURRENCY [QUOTE_CURRENCY ...]]
                              [-a] [--trading-mode {spot,margin,futures}]
usage: chaindrift list-pairs [-h] [-v] [--logfile FILE] [-V] [-c PATH]
                            [-d PATH] [--userdir PATH] [--exchange EXCHANGE]
                            [--print-list] [--print-json] [-1] [--print-csv]
                            [--base BASE_CURRENCY [BASE_CURRENCY ...]]
...
```
$ chaindrift list-pairs --quote USD --print-json
```
...
```
$ chaindrift list-pairs -c config_binance.json --all --base BTC ETH --quote USDT USD --print-list
```

* Print all markets on exchange "Kraken", in the tabular format:

```
$ chaindrift list-markets --exchange kraken --all
```
...
```
usage: chaindrift test-pairlist [-h] [--userdir PATH] [-v] [-c PATH]
                               [--quote QUOTE_CURRENCY [QUOTE_CURRENCY ...]]
...
```
chaindrift test-pairlist --config config.json --quote USDT BTC
```

## Convert database

`chaindrift convert-db` can be used to convert your database from one system to another (sqlite -> postgres, postgres -> other postgres), migrating all trades, orders and Pairlocks.
...
```
usage: chaindrift convert-db [-h] [--db-url PATH] [--db-url-from PATH]
...
!!! Warning
    Please ensure to only use this on an empty target database. Chaindrift will perform a regular migration, but may fail if entries already existed.
...
Run chaindrift in webserver mode.
Chaindrift will start the webserver and allow ChainUI to start and control backtesting processes.
...
ChainUI will also show the backtesting results.

```
usage: chaindrift webserver [-h] [-v] [--logfile FILE] [-V] [-c PATH] [-d PATH]
                           [--userdir PATH]
...
You can use `docker compose run --rm -p 127.0.0.1:8080:8080 chaindrift webserver` to start a one-off container that'll be removed once you stop it. This assumes that port 8080 is still available and no other bot is running on that port.
...
```
usage: chaindrift backtesting-show [-h] [-v] [--logfile FILE] [-V] [-c PATH]
                                  [-d PATH] [--userdir PATH]
...
```
usage: chaindrift backtesting-analysis [-h] [-v] [--logfile FILE] [-V]
...
```
usage: chaindrift hyperopt-list [-h] [-v] [--logfile FILE] [-V] [-c PATH]
...
```
chaindrift hyperopt-list --profitable --no-details
```
...
```
usage: chaindrift hyperopt-show [-h] [-v] [--logfile FILE] [-V] [-c PATH]
                               [-d PATH] [--userdir PATH] [--best]
...
```
chaindrift hyperopt-show -n 168
```

Prints JSON data with details for the last best epoch (i.e., the best of all epochs):

```
chaindrift hyperopt-show --best -n -1 --print-json --no-header
```
...
```
usage: chaindrift show-trades [-h] [-v] [--logfile FILE] [-V] [-c PATH]
...
``` bash
chaindrift show-trades --db-url sqlite:///tradesv3.sqlite --trade-ids 2 3 --print-json
```
...
```
usage: chaindrift strategy-updater [-h] [-v] [--logfile FILE] [-V] [-c PATH]
                                  [-d PATH] [--userdir PATH]
