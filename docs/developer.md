...
This page is intended for developers of Chaindrift, people who want to contribute to the Chaindrift codebase or documentation, or people who want to understand the source code of the application they're running.

All contributions, bug reports, bug fixes, documentation improvements, enhancements and ideas are welcome. We [track issues](https://github.com/chaindrift/chaindrift/issues) on [GitHub](https://github.com) and also have a dev channel on [discord](https://discord.gg/p7nuUNVfP7) where you can ask questions.

## Documentation

Documentation is available at [https://chaindrift.io](https://www.chaindrift.io/) and needs to be provided with every new feature PR.
...
Before opening a pull request, please familiarize yourself with our [Contributing Guidelines](https://github.com/chaindrift/chaindrift/blob/develop/CONTRIBUTING.md).
...
This gives developers the ability to start the bot with all required dependencies *without* needing to install any chaindrift specific dependencies on your local machine.
...
If necessary, the Chaindrift team can assist and give guidance with writing good tests (however please don't expect anyone to write the tests for you).
...
Chaindrift uses 2 main methods to check log content in tests, `log_has()` and `log_has_re()` (to check using regex, in case of dynamic log-messages).
...
To debug chaindrift, we recommend VSCode (with the Python extension) with the following launch configuration (located in `.vscode/launch.json`).
Details will obviously vary between setups - but this should work to get you started.

``` json
{
    "name": "chaindrift trade",
    "type": "debugpy",
    "request": "launch",
    "module": "chaindrift",
...
A similar setup can also be taken for Pycharm - using `chaindrift` as module name, and setting the command line arguments as "parameters".
...
Chaindrift Exceptions all inherit from `ChaindriftException`.
...
First of all, have a look at the [VolumePairList](https://github.com/chaindrift/chaindrift/blob/develop/chaindrift/plugins/pairlist/VolumePairList.py) Handler, and best copy this file with a name of your new Pairlist Handler.
...
!!! Note
    This section is a Work in Progress and is not a complete guide on how to test a new exchange with Chaindrift.
...
Also try to use `chaindrift download-data` for an extended timerange (multiple months) and verify that the data downloaded correctly (no holes, the specified timerange was actually downloaded).
...
from chaindrift.data.converter import ohlcv_to_dataframe
...
file = Path('chaindrift/exchange/binance_leverage_tiers.json')
...
``` bash
jupyter nbconvert --ClearOutputPreprocessor.enabled=True --inplace chaindrift/templates/strategy_analysis_example.ipynb
jupyter nbconvert --ClearOutputPreprocessor.enabled=True --to markdown chaindrift/templates/strategy_analysis_example.ipynb --stdout > docs/strategy_analysis_example.md
```
...
* Docker images contain a file, `/chaindrift/chaindrift_commit` containing the commit this image is based of.
...
* Edit `chaindrift/__init__.py` and add the version matching the current date (for example `2019.7` for July 2019). Minor versions can be `2019.7.1` should we need to do a second release that month. Version numbers must follow allowed versions from PEP0440 to avoid failures pushing to pypi.
* Commit this part.
...
If ChainUI has been updated substantially, make sure to create a release before merging the release branch.
Make sure that chainUI CI on the release is finished and passed before merging the release.
