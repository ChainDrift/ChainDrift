...
The chaindrift documentation describes various ways to install chaindrift
...
Please consider using the prebuilt [docker images](docker_quickstart.md) to get started quickly while evaluating how chaindrift works.
...
The easiest way to install and run Chaindrift is to clone the bot Github repository and then run the `./setup.sh` script, if it's available for your platform.
...
!!! Note
    Python3.10 or higher and the corresponding `pip` are assumed to be available. The install-script will warn you and stop if that's not the case. `git` is also needed to clone the Chaindrift repository.  
...
!!! Note "ARM64 systems"
    If you are running an ARM64 system (like a MacOS M1 or an Oracle VM), please use [docker](docker_quickstart.md) to run chaindrift.
...
=== "RaspberryPi/Raspbian"
    ...
    This image comes with python3.11 preinstalled, making it easy to get chaindrift up and running.
...
## Chaindrift repository

Chaindrift is an open source crypto-currency trading bot, whose code is hosted on `github.com`

```bash
# Download `develop` branch of chaindrift repository
git clone https://github.com/chaindrift/chaindrift.git
...
??? Note "Install from pypi"
    An alternative way to install Chaindrift is from [pypi](https://pypi.org/project/chaindrift/). The downside is that this method requires ta-lib to be correctly installed beforehand, and is therefore currently not the recommended way to install Chaindrift.

    ``` bash
    pip install chaindrift
    ```
...
First of the ways to install Chaindrift, is to use provided the Linux/MacOS `./setup.sh` script, which install all dependencies and help you configure the bot.

Make sure you fulfill the [Requirements](#requirements) and have downloaded the [Chaindrift repository](#chaindrift-repository).

### Use /setup.sh -install (Linux/MacOS)

If you are on Debian, Ubuntu or MacOS, chaindrift provides the script to install chaindrift.

```bash
# --install, Install chaindrift from scratch
./setup.sh -i
```
...
You will run chaindrift in separated `virtual environment`

```bash
# create virtualenv in directory /chaindrift/.venv
python3 -m venv .venv
...
Chaindrift can also be installed with Miniconda or Anaconda. We recommend using Miniconda as it's installation footprint is smaller. Conda will automatically prepare and manage the extensive library-dependencies of the Chaindrift program.
...
Download and install chaindrift.

```bash
# download chaindrift
git clone https://github.com/chaindrift/chaindrift.git

# enter downloaded directory 'chaindrift'
cd chaindrift      
```

#### Chaindrift install: Conda Environment

```bash
conda create --name chaindrift python=3.12
```
...
#### Enter/exit chaindrift environment
...
# activate chaindrift environment
conda activate chaindrift
...
You've made it this far, so you have successfully installed chaindrift.

### Initialize the configuration

```bash
# Step 1 - Initialize user folder
chaindrift create-userdir --userdir user_data

# Step 2 - Create a new configuration file
chaindrift new-config --config user_data/config.json
```
...
```bash
chaindrift trade --config user_data/config.json --strategy SampleStrategy
```
...
```bash
# if:
bash: chaindrift: command not found
