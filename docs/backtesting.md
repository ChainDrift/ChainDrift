...
```
usage: chaindrift backtesting [-h] [-v] [--logfile FILE] [-V] [-c PATH]
                             [-d PATH] [--userdir PATH] [-s NAME]
                             [--strategy-path PATH]
                             [--recursive-strategy-search]
                             [--chainaimodel NAME] [--chainaimodel-path PATH]
...
If no data is available for the exchange / pair / timeframe combination, backtesting will ask you to download them first using `chaindrift download-data`.
For details on downloading, please refer to the [Data Downloading](data-download.md) section in the documentation.

The result of backtesting will confirm if your bot has better odds of making a profit than a loss.

All profit calculations include fees, and chaindrift will use the exchange's default fees for the calculation.
...
!!! Note
    By default, Chaindrift will export backtesting results to `user_data/backtest_results`.
    The exported trades can be used for [further analysis](#further-backtest-result-analysis) or can be used by the [plotting sub-command](plotting.md#plot-price-and-indicators) (`chaindrift plot-dataframe`) in the scripts directory.
...
```bash
chaindrift backtesting --strategy AwesomeStrategy
```
...
```bash
chaindrift backtesting --strategy AwesomeStrategy --timeframe 1m
```
...
```bash
chaindrift backtesting --strategy AwesomeStrategy --dry-run-wallet 1000
```
...
```bash
chaindrift backtesting --strategy AwesomeStrategy --datadir user_data/data/binance-20180101 
```
...
```bash
chaindrift backtesting --strategy-list SampleStrategy1 AwesomeStrategy --timeframe 5m
```
...
```bash
chaindrift backtesting --strategy backtesting --export none --config config.json 
```
...
```bash
chaindrift backtesting --strategy backtesting --export trades --export-filename=backtest_samplestrategy.json
```
...
```bash
chaindrift backtesting --fee 0.001
```
...
```bash
chaindrift backtesting --timerange=20190501-
```
...
``` bash
chaindrift backtesting --strategy MyAwesomeStrategy --breakdown day week
```
...
!!! Warning
    Caching is automatically disabled for open-ended timeranges (`--timerange 20210101-`), as chaindrift cannot ensure reliably that the underlying data didn't change. It can also use cached results where it shouldn't if the original backtest had missing data at the end, which was fixed by downloading more data.
    In this instance, please use `--cache none` once to force a fresh backtest.

### Further backtest-result analysis

To further analyze your backtest results, chaindrift will export the trades to file by default.
...
Chaindrift has however no information about historic limits.
...
To mitigate this, chaindrift can use a lower (faster) timeframe to simulate intra-candle movements.

To utilize this, you can append `--timeframe-detail 5m` to your regular backtesting command.

``` bash
chaindrift backtesting --strategy AwesomeStrategy --timeframe 1h --timeframe-detail 5m
```
...
``` bash
chaindrift backtesting --timerange 20180401-20180410 --timeframe 5m --strategy-list Strategy001 Strategy002 --export trades
```
