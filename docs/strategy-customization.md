...
- the [Chaindrift strategy 101](strategy-101.md), which provides a quick start to strategy development
- the [Chaindrift bot basics](bot-basics.md), which provides overall info on how the bot operates

## Develop your own strategy

The bot includes a default strategy file.

Also, several other strategies are available in the [strategy repository](https://github.com/chaindrift/chaindrift-strategies).
...
```bash
chaindrift new-strategy --strategy AwesomeStrategy
```
...
!!! Note
    There is a difference between the *name* of the strategy and the filename. In most commands, Chaindrift uses the *name* of the strategy, *not the filename*.
...
??? Hint "Different template levels"
    `chaindrift new-strategy` has an additional parameter, `--template`, which controls the amount of pre-build information you get in the created strategy. Use `--template minimal` to get an empty strategy without any indicator examples, or `--template advanced` to get a template with more complicated features defined.
...
```bash
chaindrift trade --strategy AwesomeStrategy
```

### Bot modes

Chaindrift strategies can be processed by the Chaindrift bot in 5 main modes:

- backtesting
- hyperopting
- dry ("forward testing")
- live
- ChainAI (not covered here)
...
## Diving in deeper
**For the following section we will use the [user_data/strategies/sample_strategy.py](https://github.com/chaindrift/chaindrift/blob/develop/chaindrift/templates/sample_strategy.py)
file as reference.**
...
??? Hint "Lookahead and recursive analysis"
    Chaindrift includes two helpful commands to help assess common lookahead (using future data) and 
...
Chaindrift uses [pandas](https://pandas.pydata.org/) to store/provide the candlestick (OHLCV) data.
...
Chaindrift uses this new column as an entry signal, where it is assumed that a trade will subsequently open on the next open candle.
...
Chaindrift does not store incomplete/unfinished candles in the dataframe.
...
Chaindrift does not. Only complete/finished candle data is available in the dataframe.
...
!!! Note "Want more indicator examples?"
    Look into the [user_data/strategies/sample_strategy.py](https://github.com/chaindrift/chaindrift/blob/develop/chaindrift/templates/sample_strategy.py).
    Then uncomment indicators you need.

#### Indicator libraries

Out of the box, chaindrift installs the following technical libraries:

- [ta-lib](https://ta-lib.github.io/ta-lib-python/)
- [pandas-ta](https://twopirllc.github.io/pandas-ta/)
- [technical](https://technical.chaindrift.io)

Additional technical libraries can be installed as necessary, or custom indicators may be written / invented by the strategy author.

### Strategy startup period

Some indicators have an unstable startup period in which there isn't enough candle data to calculate any values (NaN), or the calculation is incorrect. This can lead to inconsistencies, since Chaindrift does not know how long this unstable period is and uses whatever indicator values are in the dataframe.
...
!!! Warning "Using x calls to get OHLCV"
    ...
    Having this will cause Chaindrift to make multiple calls for the same pair, which will obviously be slower than one network request.
    As a consequence, Chaindrift will take longer to refresh candles - and should therefore be avoided if possible.
    This is capped to 5 total calls to avoid overloading the exchange, or make chaindrift too slow.
...
``` bash
chaindrift backtesting --timerange 20190101-20190201 --timeframe 5m
```
...
``` python
from chaindrift.exchange import timeframe_to_minutes
...
By default, chaindrift will attempt to load strategies from all `.py` files within the `userdir` (default `user_data/strategies`).

Assuming your strategy is called `AwesomeStrategy`, stored in the file `user_data/strategies/AwesomeStrategy.py`, then you can start chaindrift in dry (or live, depending on your configuration) mode with:
...
You can use `chaindrift list-strategies` to see a list of all strategies Chaindrift is able to load (all strategies in the correct folder).
...
??? Example "Fast and easy way to define informative pairs"
    ...
    from chaindrift.strategy import IStrategy, informative
...
??? Note "Plotting with current_whitelist"
    ...
    It's also not supported for ChainUI visualization in [webserver mode](utils.md#webserver-mode), as the configuration for webserver mode doesn't require a pairlist to be set.
...
This method is used by chaindrift internally to determine the last signal.
...
```python
from chaindrift.strategy import IStrategy, merge_informative_pair
...
Chaindrift locks pairs automatically for the current candle (until that candle is over) when a pair exits, preventing an immediate re-entry of that pair.
...
Chaindrift has an easy method to do this from within the strategy, by calling `self.lock_pair(pair, until, [reason])`.
...
When conflicting signals collide (e.g. both `'enter_long'` and `'exit_long'` are set to `1`), chaindrift will do nothing and ignore the entry signal. This will avoid trades that enter, and exit immediately. Obviously, this can potentially lead to missed entries.
...
To get additional ideas for strategies, head over to the [strategy repository](https://github.com/chaindrift/chaindrift-strategies). Feel free to use them as examples, but results will depend on the current market situation, pairs used, etc. Therefore, these strategies should be considered only for learning purposes, not real world trading. Please backtest the strategy for your exchange/desired pairs first, then dry run to evaluate carefully, and use at your own risk.
