...
from chaindrift.configuration.deprecated_settings import process_temporary_deprecated_settings
from chaindrift.configuration.directory_operations import create_datadir, create_userdata_dir
from chaindrift.configuration.environment_vars import enironment_vars_to_dict
from chaindrift.configuration.load_config import load_file, load_from_files
...
from chaindrift.misc import deep_merge_dicts, parse_db_uri_for_logging
...
class Configuration:
    ...
    def load_config(self) -> dict[str, Any]:
        ...
        from chaindrift.commands.arguments import NO_CONF_ALLOWED
        ...
        self._process_chainai_options(config)

        # Import check_exchange here to avoid import cycle problems
        from chaindrift.exchange.check_exchange import check_exchange
    ...
    def _process_optimize_options(self, config: Config) -> None:
        ...
        configurations = [
            ...
            ("chainai_backtest_live_models", "Parameter --chainai-backtest-live-models detected ..."),
        ]
        self._args_to_config_loop(config, configurations)
    ...
    def _process_chainai_options(self, config: Config) -> None:
        self._args_to_config(
            config, argname="chainaimodel", logstring="Using chainaimodel class name: {}"
        )

        self._args_to_config(
            config, argname="chainaimodel_path", logstring="Using chainaimodel path: {}"
        )
