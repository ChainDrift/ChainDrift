...
from chaindrift.configuration.config_schema import (
    CONF_SCHEMA,
...
from chaindrift.configuration.deprecated_settings import process_deprecated_setting
from chaindrift.constants import UNLIMITED_STAKE_AMOUNT
from chaindrift.enums import RunMode, TradingMode
...
def _extend_validator(validator_class):
    """
    Extended validator for the Chaindrift configuration JSON Schema.
...
ChaindriftValidator = _extend_validator(Draft4Validator)
...
def validate_config_schema(conf: dict[str, Any], preliminary: bool = False) -> dict[str, Any]:
    ...
    try:
        ChaindriftValidator(conf_schema).validate(conf)
        return conf
...
def validate_config_consistency(conf: dict[str, Any], *, preliminary: bool = False) -> None:
    ...
    _validate_chainai_hyperopt(conf)
    _validate_chainai_backtest(conf)
    _validate_chainai_include_timeframes(conf, preliminary=preliminary)
...
def _validate_chainai_hyperopt(conf: dict[str, Any]) -> None:
    chainai_enabled = conf.get("chainai", {}).get("enabled", False)
    analyze_per_epoch = conf.get("analyze_per_epoch", False)
    if analyze_per_epoch and chainai_enabled:
        raise ConfigurationError(
            "Using analyze-per-epoch parameter is not supported with a ChainAI strategy."
        )


def _validate_chainai_include_timeframes(conf: dict[str, Any], preliminary: bool) -> None:
    chainai_enabled = conf.get("chainai", {}).get("enabled", False)
    if chainai_enabled:
        main_tf = conf.get("timeframe", "5m")
        chainai_include_timeframes = (
            conf.get("chainai", {}).get("feature_parameters", {}).get("include_timeframes", [])
        )
        ...
        offending_lines = []
        for tf in chainai_include_timeframes:
            tf_s = timeframe_to_seconds(tf)
            if tf_s < main_tf_s:
                offending_lines.append(tf)
        if offending_lines:
            raise ConfigurationError(
                f"Main timeframe of {main_tf} must be smaller or equal to ChainAI "
                f"`include_timeframes`.Offending include-timeframes: {', '.join(offending_lines)}"
            )

        # Ensure that the base timeframe is included in the include_timeframes list
        if not preliminary and main_tf not in chainai_include_timeframes:
            feature_parameters = conf.get("chainai", {}).get("feature_parameters", {})
            include_timeframes = [main_tf] + chainai_include_timeframes
            conf.get("chainai", {}).get("feature_parameters", {}).update(
                {**feature_parameters, "include_timeframes": include_timeframes}
            )


def _validate_chainai_backtest(conf: dict[str, Any]) -> None:
    if conf.get("runmode", RunMode.OTHER) == RunMode.BACKTEST:
        chainai_enabled = conf.get("chainai", {}).get("enabled", False)
        timerange = conf.get("timerange")
        chainai_backtest_live_models = conf.get("chainai_backtest_live_models", False)
        if chainai_backtest_live_models and chainai_enabled and timerange:
            raise ConfigurationError(
                "Using timerange parameter is not supported with "
                "--chainai-backtest-live-models parameter."
            )

        if chainai_backtest_live_models and not chainai_enabled:
            raise ConfigurationError(
                "Using --chainai-backtest-live-models parameter is only "
                "supported with a ChainAI strategy."
            )

        if chainai_enabled and not chainai_backtest_live_models and not timerange:
            raise ConfigurationError(
                "Please pass --timerange if you intend to use ChainAI for backtesting."
            )
