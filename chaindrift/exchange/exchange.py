...
from chaindrift.constants import (
...
from chaindrift.data.converter import (
...
from chaindrift.exchange.common import (
    API_FETCH_ORDER_RETRY_COUNT,
    remove_exchange_credentials,
    retrier,
    retrier_async,
)
from chaindrift.exchange.exchange_types import (
    CcxtBalances,
    CcxtOrder,
    CcxtPosition,
    FtHas,
...
from chaindrift.exchange.exchange_ws import ExchangeWS
from chaindrift.misc import (
    chunks,
    deep_merge_dicts,
    file_dump_json,
    file_load_json,
...
from chaindrift.util import dt_from_ts, dt_now
from chaindrift.util.datetime_helpers import dt_humanize_delta, dt_ts, format_ms_time
...
class Exchange:
    ...
    def market_is_tradable(self, market: dict[str, Any]) -> bool:
        """
        Check if the market symbol is tradable by Chaindrift.
    ...
    def validate_timeframes(self, timeframe: str | None) -> None:
        ...
        if (
            ...
            raise ConfigurationError("Timeframes < 1m are currently not supported by Chaindrift.")
    ...
    def validate_chainai(self, config: Config) -> None:
        chainai_enabled = config.get("chainai", {}).get("enabled", False)
        if chainai_enabled and not self._ft_has["ohlcv_has_history"]:
            raise ConfigurationError(
                f"Historic OHLCV data not available for {self.name}. Can't use chainAI."
            )
    ...
    def validate_trading_mode_and_margin_mode(
        ...
        """
        Checks if chaindrift can perform trades using the configured
        trading mode(Margin, Futures) and MarginMode(Cross, Isolated)
        Throws OperationalException:
            If the trading_mode/margin_mode type are not supported by chaindrift on this exchange
        """
        if trading_mode != TradingMode.SPOT and (
            ...
            raise OperationalException(
                f"Chaindrift does not support {mm_value} {trading_mode} on {self.name}"
            )
    ...
    def fetch_dry_run_order(self, order_id) -> CcxtOrder:
        ...
        try:
            ...
            from chaindrift.persistence import Order
    ...
    def dry_run_liquidation_price(
        ...
        if self.trading_mode == TradingMode.FUTURES and self.margin_mode == MarginMode.ISOLATED:
            if market["inverse"]:
                raise OperationalException("Chaindrift does not yet support inverse contracts")
            ...
            raise OperationalException(
                "Chaindrift only supports isolated futures for leverage trading"
            )
