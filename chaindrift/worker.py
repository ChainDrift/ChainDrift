...
from chaindrift.constants import PROCESS_THROTTLE_SECS, RETRY_TIMEOUT, Config
from chaindrift.enums import RPCMessageType, State
from chaindrift.exceptions import OperationalException, TemporaryError
from chaindrift.exchange import timeframe_to_next_date
...
class Worker:
    ...
    def _init(self, reconfig: bool) -> None:
        ...
        self.chaindrift = ChaindriftBot(self._config)
    ...
    def _worker(self, old_state: State | None) -> State:
        ...
        """
        state = self.chaindrift.state

        # Log state transition
        if state != old_state:
            if old_state != State.RELOAD_CONFIG:
                self.chaindrift.notify_status(f"{state.name.lower()}")

            logger.info(
                f"Changing state{f' from {old_state.name}' if old_state else ''} to: {state.name}"
            )
            if state == State.RUNNING:
                self.chaindrift.startup()

            if state == State.STOPPED:
                self.chaindrift.check_for_open_trades()
        ...
        if self._heartbeat_interval:
            ...
            if (now - self._heartbeat_msg) > self._heartbeat_interval:
                version = __version__
                strategy_version = self.chaindrift.strategy.version()
    ...
    def _process_stopped(self) -> None:
        self.chaindrift.process_stopped()

    def _process_running(self) -> None:
        try:
            self.chaindrift.process()
            ...
            self.chaindrift.notify_status(
                f"*OperationalException:*\n```\n{tb}```\n {hint}", msg_type=RPCMessageType.EXCEPTION
            )

            logger.exception("OperationalException. Stopping trader ...")
            self.chaindrift.state = State.STOPPED

    def _reconfigure(self) -> None:
        """
        Cleans up current chaindriftbot instance, reloads the configuration and
        replaces it with the new instance
        """
        # Tell systemd that we initiated reconfiguration
        self._notify("RELOADING=1")

        # Clean up current chaindrift modules
        self.chaindrift.cleanup()

        # Load and validate config and create new instance of the bot
        self._init(True)

        self.chaindrift.notify_status("config reloaded")
    ...
    def exit(self) -> None:
        ...
        if self.chaindrift:
            self.chaindrift.notify_status("process died")
            self.chaindrift.cleanup()
