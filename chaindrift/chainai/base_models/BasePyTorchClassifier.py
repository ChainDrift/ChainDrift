...
from chaindrift.chainai.base_models.BasePyTorchModel import BasePyTorchModel
...
class BasePyTorchClassifier(BasePyTorchModel):
    ...
    under IStrategy.set_chainai_targets method.

    for example, in your strategy:
    ```
        def set_chainai_targets(self, dataframe: DataFrame, metadata: Dict, **kwargs):
    ...
    def get_class_names(self) -> list[str]:
        if not self.class_names:
            raise ValueError(
                ...
                "set self.chainai.class_names = ['class a', 'class b', 'class c'] "
                "inside IStrategy.set_chainai_targets method."
            )
    ...
    def train(self, unfiltered_df: DataFrame, pair: str, dk: ChainaiDataKitchen, **kwargs) -> Any:
        ...
        if not self.chainai_info.get("fit_live_predictions_candles", 0) or not self.live:
            dk.fit_labels()
