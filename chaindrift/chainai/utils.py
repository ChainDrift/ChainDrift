...
from chaindrift.data.history.history_utils import refresh_backtest_ohlcv_data
...
from chaindrift.plugins.pairlist.pairlist_helpers import dynamic_expand_pairlist
...
def download_all_data_for_training(dp: DataProvider, config: Config) -> None:
    ...
    refresh_backtest_ohlcv_data(
        dp._exchange,
        pairs=all_pairs,
        timeframes=config["chainai"]["feature_parameters"].get("include_timeframes"),
...
def get_required_data_timerange(config: Config) -> TimeRange:
    ...
    for auto data-download in ChainAI
    """
    ...
    indicator_periods = config["chainai"]["feature_parameters"]["indicator_periods_candles"]
    ...
    logger.info(f"ChainAI auto-downloader using {max_period} startup candles.")

    additional_seconds = max_period * max_tf_seconds

    startts = int(time - config["chainai"].get("train_period_days", 0) * 86400 - additional_seconds)
    stopts = int(time)
...
def plot_feature_importance(
    model: Any, pair: str, dk: ChainaiDataKitchen, count_max: int = 25
) -> None:
    """
    ...
    """
    from chaindrift.plot.plotting import go, make_subplots, store_plot_file

    # Extract feature importance from model
    models = {}
    if "ChainaiMultiOutputRegressor" in str(model.__class__):
    ...
    for label in models:
        ...
        label = label.replace("&", "").replace("%", "")  # escape two ChainAI specific characters
...
def get_timerange_backtest_live_models(config: Config) -> str:
    ...
    """
    dk = ChainaiDataKitchen(config)
    models_path = dk.get_full_models_path(config)
    dd = ChainaiDataDrawer(models_path, config)
...
def get_tb_logger(model_type: str, path: Path, activate: bool) -> Any:
    if model_type == "pytorch" and activate:
        from chaindrift.chainai.tensorboard import TBLogger

        return TBLogger(path, activate)
    else:
        from chaindrift.chainai.tensorboard.base_tensorboard import BaseTensorboardLogger
