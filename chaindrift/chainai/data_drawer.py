...
class ChainaiDataDrawer:
    ...
    def __init__(self, full_path: Path, config: Config):
        self.config = config
        self.chainai_info = config.get("chainai", {})
        ...
        self.model_type = self.chainai_info.get("model_save_type", "joblib")
        ...
        if exists:
            try:
                ...
                with self.historic_predictions_bkp_path.open("rb") as fp:
                    self.historic_predictions = cloudpickle.load(fp)
                logger.warning("ChainAI successfully loaded the backup historical predictions file.")
    ...
    def set_initial_return_values(
        ...
        We also aim to keep the date from historical predictions so that the ChainUI displays
        zeros during any downtime (between ChainAI reloads).
        """
        ...
        if len(common_dates.index) > 0:
            ...
            logger.warning(
                ...
                "predictions. You likely left your ChainAI instance offline "
    ...
    def append_model_predictions(
        ...
        the tail is equivalent to the length of the dataframe that entered ChainAI from
        the strategy originally. Doing this allows ChainUI to always display the correct
    ...
    def return_null_values_to_strategy(self, dataframe: DataFrame, dk: ChainaiDataKitchen) -> None:
        """
    ...
    def purge_old_models(self) -> None:
        num_keep = self.chainai_info["purge_old_models"]
        if not num_keep:
            return
            ...
            if delete_dict[coin]["num_folders"] > num_keep:
                ...
                for k, v in sorted_dict.items():
                    if deleted >= num_delete:
                        break
                    logger.info(f"Chainai purging old model file {v}")
                    shutil.rmtree(v)
                    deleted += 1

    def save_metadata(self, dk: ChainaiDataKitchen) -> None:
        """
    ...
    def save_data(self, model: Any, coin: str, dk: ChainaiDataKitchen) -> None:
        """
    ...
    def load_metadata(self, dk: ChainaiDataKitchen) -> None:
        """
    ...
    def load_data(self, coin: str, dk: ChainaiDataKitchen) -> Any:  # noqa: C901
        """
        ...
        if dk.live and coin in self.model_dictionary:
            ...
            mod = importlib.import_module(
                self.model_type, self.chainai_info["rl_config"]["model_type"]
            )
            MODELCLASS = getattr(mod, self.chainai_info["rl_config"]["model_type"])
    ...
    def update_historic_data(self, strategy: IStrategy, dk: ChainaiDataKitchen) -> None:
        """
        ...
        """
        feat_params = self.chainai_info["feature_parameters"]
    ...
    def load_all_pair_histories(self, timerange: TimeRange, dk: ChainaiDataKitchen) -> None:
        """
        ...
        for pair in dk.all_pairs:
            if pair not in history_data:
                history_data[pair] = {}
            for tf in self.chainai_info["feature_parameters"].get("include_timeframes"):
    ...
    def get_base_and_corr_dataframes(
        self, timerange: TimeRange, pair: str, dk: ChainaiDataKitchen
        ...
        with self.history_lock:
            ...
            pairs = self.chainai_info["feature_parameters"].get("include_corr_pairlist", [])
    ...
    def get_timerange_from_live_historic_predictions(self) -> TimeRange:
        ...
        if not self.historic_predictions_path.is_file():
            raise OperationalException(
                ...
                "to run backtest with the chainai-backtest-live-models option "
            )
