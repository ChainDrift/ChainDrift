...
from chaindrift.chainai.RL.Base5ActionRLEnv import Actions, Base5ActionRLEnv
from chaindrift.chainai.RL.BaseEnvironment import BaseActions, BaseEnvironment, Positions
from chaindrift.chainai.tensorboard.TensorboardCallback import TensorboardCallback
...
class BaseReinforcementLearningModel(IChainaiModel):
    """
    ...
    def __init__(self, **kwargs) -> None:
        ...
        self.max_threads = min(
            self.chainai_info["rl_config"].get("cpu_count", 1),
            max(int(self.max_system_threads / 2), 1),
        )
        th.set_num_threads(self.max_threads)
        self.reward_params = self.chainai_info["rl_config"]["model_reward_parameters"]
        ...
        self.rl_config = self.chainai_info["rl_config"]
        ...
        self.policy_type = self.chainai_info["rl_config"]["policy_type"]
    ...
    def unset_outlier_removal(self):
        ...
        if self.chainai_info["data_split_parameters"].get("shuffle", False):
            self.chainai_info["data_split_parameters"].update({"shuffle": False})
    ...
    def train(self, unfiltered_df: DataFrame, pair: str, dk: ChainaiDataKitchen, **kwargs) -> Any:
        ...
        if self.chainai_info.get("data_split_parameters", {}).get("test_size", 0.1) != 0:
    ...
    def set_train_and_eval_environments(
        ...
        :param dk: ChainaiDataKitchen = the datakitchen for the current pair
        """
    ...
    @abstractmethod
    def fit(self, data_dictionary: dict[str, Any], dk: ChainaiDataKitchen, **kwargs):
        """
    ...
    def rl_model_predict(
        self, dataframe: DataFrame, dk: ChainaiDataKitchen, model: Any
    ) -> DataFrame:
        """
        A helper function to make predictions in the Reinforcement learning module.
        :param dataframe: DataFrame = the dataframe of features to make the predictions on
        :param dk: ChainaiDatakitchen = data kitchen for the current pair
    ...
    def build_ohlc_price_dataframes(
        self, data_dictionary: dict, pair: str, dk: ChainaiDataKitchen
    ...
    def drop_ohlc_from_df(self, df: DataFrame, dk: ChainaiDataKitchen):
        """
    ...
    def load_model_from_disk(self, dk: ChainaiDataKitchen) -> Any:
        """
