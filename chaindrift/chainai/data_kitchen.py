...
from chaindrift.data.converter import reduce_dataframe_footprint
from chaindrift.exceptions import OperationalException
from chaindrift.exchange import timeframe_to_seconds
from chaindrift.strategy import merge_informative_pair
...
class ChainaiDataKitchen:
    """
    Class designed to analyze data for a single pair. Employed by the IChainaiModel class.
    ...
    def __init__(
        ...
        self.config = config
        self.chainai_config: dict[str, Any] = config["chainai"]
        ...
        self.live = live
        self.pair = pair
        self.keras: bool = self.chainai_config.get("keras", False)
        self.set_all_pairs()
        self.backtest_live_models = config.get("chainai_backtest_live_models", False)
        ...
        if not self.live:
            ...
            if not self.backtest_live_models:
                self.full_timerange = self.create_fulltimerange(
                    self.config["timerange"], self.chainai_config.get("train_period_days", 0)
                )
                (self.training_timeranges, self.backtesting_timeranges) = self.split_timerange(
                    self.full_timerange,
                    config["chainai"]["train_period_days"],
                    config["chainai"]["backtest_period_days"],
                )

        self.data["extra_returns_per_train"] = self.chainai_config.get("extra_returns_per_train", {})
        if not self.chainai_config.get("data_kitchen_thread_count", 0):
            self.thread_count = max(int(psutil.cpu_count() * 2 - 2), 1)
        else:
            self.thread_count = self.chainai_config["data_kitchen_thread_count"]
    ...
    def make_train_test_datasets(
        ...
        """
        feat_dict = self.chainai_config["feature_parameters"]

        if "shuffle" not in self.chainai_config["data_split_parameters"]:
            self.chainai_config["data_split_parameters"].update({"shuffle": False})
        ...
        if self.chainai_config.get("data_split_parameters", {}).get("test_size", 0.1) != 0:
            (
                train_features,
                test_features,
                train_labels,
                ...
                labels,
                weights,
                **self.config["chainai"]["data_split_parameters"],
            )
        else:
        ...
        if self.chainai_config["feature_parameters"].get("reverse_train_test_order", False):
    ...
    def set_weights_higher_recent(self, num_weights: int) -> npt.ArrayLike:
        ...
        """
        wfactor = self.config["chainai"]["feature_parameters"]["weight_factor"]
    ...
    def get_predictions_to_append(
        ...
        if self.chainai_config["feature_parameters"].get("DI_threshold", 0) > 0:
    ...
    def create_fulltimerange(self, backtest_tr: str, backtest_period_days: int) -> str:
        ...
        if backtest_timerange.stopts == 0:
            ...
            raise OperationalException(
                "ChainAI backtesting does not allow open ended timeranges. "
    ...
    def check_if_model_expired(self, trained_timestamp: int) -> bool:
        ...
        max_time = self.chainai_config.get("expiration_hours", 0)
        if max_time > 0:
    ...
    def check_if_new_training_required(
        ...
        timeframes = self.chainai_config["feature_parameters"].get("include_timeframes")
        ...
        if trained_timestamp != 0:
            ...
            retrain = elapsed_time > self.chainai_config.get("live_retrain_hours", 0)
            if retrain:
                trained_timerange.startts = int(
                    time - self.chainai_config.get("train_period_days", 0) * SECONDS_IN_DAY
                )
                ...
                data_load_timerange.startts = int(
                    time
                    - self.chainai_config.get("train_period_days", 0) * SECONDS_IN_DAY
                    - additional_seconds
                )
    ...
    def set_all_pairs(self) -> None:
        self.all_pairs = copy.deepcopy(
            self.chainai_config["feature_parameters"].get("include_corr_pairlist", [])
        )
    ...
    def extract_corr_pair_columns_from_populated_indicators(
        ...
        pairs = self.chainai_config["feature_parameters"].get("include_corr_pairlist", [])
    ...
    def populate_features(
        ...
        """
        tfs: list[str] = self.chainai_config["feature_parameters"].get("include_timeframes")
        ...
        for tf in tfs:
            ...
            for t in self.chainai_config["feature_parameters"]["indicator_periods_candles"]:
            ...
            for n in range(self.chainai_config["feature_parameters"]["include_shifted_candles"] + 1):
                if n == 0:
                    continue
    ...
    def use_strategy_to_populate_indicators(  # noqa: C901
        ...
        if not new_version:
            raise OperationalException(
                ...
                f"{DOCS_LINK}/strategy_migration/#chainai-strategy \n"
                "And the feature_engineering_* documentation: \n"
                f"{DOCS_LINK}/chainai-feature-engineering/"
            )

        tfs: list[str] = self.chainai_config["feature_parameters"].get("include_timeframes")
        pairs: list[str] = self.chainai_config["feature_parameters"].get("include_corr_pairlist", [])
        ...
        corr_pairs: list[str] = self.chainai_config["feature_parameters"].get(
        ...
        if self.live:
            dataframe = strategy.set_chainai_targets(dataframe.copy(), metadata=metadata)
    ...
    def get_full_models_path(self, config: Config) -> Path:
        """
        Returns default ChainAI model path
        :param config: Configuration dictionary
        """
        chainai_config: dict[str, Any] = config["chainai"]
        return Path(config["user_data_dir"] / "models" / str(chainai_config.get("identifier")))
    ...
    def buffer_timerange(self, timerange: TimeRange):
        ...
        will be NaN and ChainAI will automatically cut those off of the training
        dataset.
        """
        buffer = self.chainai_config["feature_parameters"]["buffer_train_data_candles"]
        if buffer:
    ...
    def normalize_data(self, data_dictionary: dict) -> dict[Any, Any]:
        ...
        """
        logger.warning(
            f"Your custom IChainaiModel relies on the deprecated"
