...
from chaindrift.constants import DUST_PER_COIN, Config
from chaindrift.enums import MarketDirection, RPCMessageType, SignalDirection, TradingMode
from chaindrift.exceptions import OperationalException
from chaindrift.misc import chunks, plural
from chaindrift.persistence import Trade
from chaindrift.rpc import RPC, RPCException, RPCHandler
from chaindrift.rpc.rpc_types import RPCEntryMsg, RPCExitMsg, RPCOrderMsg, RPCSendMsg
from chaindrift.util import (
    dt_from_ts,
    dt_humanize_delta,
    fmt_coin,
    fmt_coin2,
...
class Telegram(RPCHandler):
    ...
    def _add_analyzed_candle(self, pair: str) -> str:
        ...
        if candle_val != "off":
            if candle_val == "ohlc":
                analyzed_df, _ = self._rpc._chaindrift.dataprovider.get_analyzed_dataframe(
    ...
    async def _help(self, update: Update, context: CallbackContext) -> None:
        ...
        if self._rpc._chaindrift.trading_mode != TradingMode.SPOT:
            force_enter_text += (
    ...
    async def _version(self, update: Update, context: CallbackContext) -> None:
        ...
        :return: None
        """
        strategy_version = self._rpc._chaindrift.strategy.version()
    ...
    async def _show_config(self, update: Update, context: CallbackContext) -> None:
        ...
        :return: None
        """
        val = RPC._rpc_show_config(self._config, self._rpc._chaindrift.state)
