...
from chaindrift.constants import CANCEL_REASON, DEFAULT_DATAFRAME_COLUMNS, Config
from chaindrift.data.history import load_data
from chaindrift.data.metrics import DrawDownResult, calculate_expectancy, calculate_max_drawdown
...
from chaindrift.exceptions import ExchangeError, PricingError
from chaindrift.exchange import timeframe_to_minutes, timeframe_to_msecs
from chaindrift.exchange.exchange_types import Tickers
from chaindrift.loggers import bufferHandler
from chaindrift.persistence import KeyStoreKeys, KeyValueStore, PairLocks, Trade
from chaindrift.persistence.models import PairLock
from chaindrift.plugins.pairlist.pairlist_helpers import expand_pairlist
from chaindrift.rpc.fiat_convert import CryptoToFiatConverter
from chaindrift.rpc.rpc_types import RPCSendMsg
from chaindrift.util import decimals_per_coin, dt_now, dt_ts_def, format_date, shorten_date
from chaindrift.util.datetime_helpers import dt_humanize_delta
from chaindrift.wallets import PositionWallet, Wallet
...
class RPC:
    ...
    def __init__(self, chaindrift) -> None:
        """
        Initializes all enabled rpc modules
        :param chaindrift: Instance of a chaindrift bot
        :return: None
        """
        self._chaindrift = chaindrift
        self._config: Config = chaindrift.config
    ...
    def _rpc_show_config(
        ...
        val = {
            ...
            "bot_name": config.get("bot_name", "chaindrift"),
    ...
    def _rpc_trade_status(self, trade_ids: list[int] | None = None) -> list[dict[str, Any]]:
        ...
        if not trades:
            ...
            for trade in trades:
                ...
                if trade.is_open:
                    try:
                        current_rate = self._chaindrift.exchange.get_rate(
                ...
                if not isnan(current_profit_abs) and self._fiat_converter:
                    current_profit_fiat = self._fiat_converter.convert_amount(
                        current_profit_abs,
                        self._chaindrift.config["stake_currency"],
                        self._chaindrift.config["fiat_display_currency"],
                    )
    ...
    def _rpc_timeunit_profit(
        ...
        daily_stake = self._chaindrift.wallets.get_total_stake_amount()
    ...
    def _rpc_trade_statistics(
        ...
        starting_balance = self._chaindrift.wallets.get_starting_balance()
    ...
    def __balance_get_est_stake(
        ...
        if coin == stake_currency:
            ...
            pair = self._chaindrift.exchange.get_valid_pair_combination(coin, stake_currency)
    ...
    def _rpc_balance(self, stake_currency: str, fiat_display_currency: str) -> dict:
        ...
        total_bot = 0.0
        try:
            tickers: Tickers = self._chaindrift.exchange.get_tickers(cached=True)
        except ExchangeError:
            raise RPCException("Error getting current tickers.")

        open_trades: list[Trade] = Trade.get_open_trades()
        open_assets: dict[str, Trade] = {t.safe_base_currency: t for t in open_trades}
        self._chaindrift.wallets.update(require_update=False)
        starting_capital = self._chaindrift.wallets.get_starting_balance()
        starting_cap_fiat = (
            self._fiat_converter.convert_amount(
        ...
        coin: str
        balance: Wallet
        for coin, balance in self._chaindrift.wallets.get_all_balances().items():
            ...
            if coin == stake_currency:
                trade_amount = self._chaindrift.wallets.get_available_stake_amount()
        ...
        for symbol, position in self._chaindrift.wallets.get_all_positions().items():
        ...
        return {
            ...
            "note": "Simulated balances" if self._chaindrift.config["dry_run"] else "",
        }

    def _rpc_start(self) -> dict[str, str]:
        """Handler for start"""
        if self._chaindrift.state == State.RUNNING:
            return {"status": "already running"}

        self._chaindrift.state = State.RUNNING
        return {"status": "starting trader ..."}

    def _rpc_stop(self) -> dict[str, str]:
        """Handler for stop"""
        if self._chaindrift.state == State.RUNNING:
            self._chaindrift.state = State.STOPPED
    ...
    def _rpc_reload_config(self) -> dict[str, str]:
        ...
        self._chaindrift.state = State.RELOAD_CONFIG
    ...
    def _rpc_stopentry(self) -> dict[str, str]:
        ...
        if self._chaindrift.state == State.RUNNING:
            ...
            self._chaindrift.config["max_open_trades"] = 0
            self._chaindrift.strategy.max_open_trades = 0
    ...
    def _rpc_reload_trade_from_exchange(self, trade_id: int) -> dict[str, str]:
        ...
        self._chaindrift.handle_onexchange_order(trade)
    ...
    def __exec_force_exit(
        ...
        for oo in trade.open_orders:
            ...
            order = self._chaindrift.exchange.fetch_order(oo.order_id, trade.pair)

            if order["side"] == trade.entry_side:
                fully_canceled = self._chaindrift.handle_cancel_enter(
            ...
            if order["side"] == trade.exit_side:
                ...
                self._chaindrift.handle_cancel_exit(trade, order, oo, CANCEL_REASON["FORCE_EXIT"])
        ...
        if all(tocr["cancel_state"] is False for tocr in trade_entry_cancelation_registry):
            ...
            order_type = ordertype or self._chaindrift.strategy.order_types.get(
                "force_exit", self._chaindrift.strategy.order_types["exit"]
            )
            sub_amount: float | None = None
            if amount and amount < trade.amount:
                # Partial exit ...
                min_exit_stake = self._chaindrift.exchange.get_min_pair_stake_amount(
            ...
            self._chaindrift.execute_trade_exit(
    ...
    def _force_entry_validations(self, pair: str, order_side: SignalDirection):
        if not self._chaindrift.config.get("force_entry_enable", False):
            raise RPCException("Force_entry not enabled.")

        if self._chaindrift.state != State.RUNNING:
            raise RPCException("trader is not running")

        if order_side == SignalDirection.SHORT and self._chaindrift.trading_mode == TradingMode.SPOT:
            raise RPCException("Can't go short on Spot markets.")

        if pair not in self._chaindrift.exchange.get_markets(tradable_only=True):
            raise RPCException("Symbol does not exist or market is not active.")
        # Check if pair quote currency equals to the stake currency.
        stake_currency = self._chaindrift.config.get("stake_currency")
        if not self._chaindrift.exchange.get_pair_quote_currency(pair) == stake_currency:
            raise RPCException(
    ...
    def _rpc_force_entry(
        ...
        *,
        ...
        if trade:
            ...
            if not self._chaindrift.strategy.position_adjustment_enable:
        ...
        if not stake_amount:
            # gen stake amount
            stake_amount = self._chaindrift.wallets.get_trade_stake_amount(
                pair, self._config["max_open_trades"]
            )

        # execute buy
        if not order_type:
            order_type = self._chaindrift.strategy.order_types.get(
                "force_entry", self._chaindrift.strategy.order_types["entry"]
            )
        with self._chaindrift._exit_lock:
            if self._chaindrift.execute_entry(
                pair,
                stake_amount,
                price,
    ...
    def _rpc_cancel_open_order(self, trade_id: int):
        ...
        with self._chaindrift._exit_lock:
            ...
            for open_order in trade.open_orders:
                try:
                    order = self._chaindrift.exchange.fetch_order(open_order.order_id, trade.pair)
                except ExchangeError as e:
                    logger.info(f"Cannot query order for {trade} due to {e}.", exc_info=True)
                    raise RPCException("Order not found.")
                self._chaindrift.handle_cancel_order(
    ...
    def _rpc_delete(self, trade_id: int) -> dict[str, str | int]:
        ...
        with self._chaindrift._exit_lock:
            ...
            for open_order in trade.open_orders:
                try:
                    self._chaindrift.exchange.cancel_order(open_order.order_id, trade.pair)
                    c_count += 1
                except ExchangeError:
                    pass
            ...
            if (
                self._chaindrift.strategy.order_types.get("stoploss_on_exchange")
                and trade.has_open_sl_orders
            ):
                for oslo in trade.open_sl_orders:
                    try:
                        self._chaindrift.exchange.cancel_stoploss_order(oslo.order_id, trade.pair)
                        c_count += 1
                    except ExchangeError:
                        pass
    ...
    def _rpc_count(self) -> dict[str, float]:
        ...
        return {
            "current": len(trades),
            "max": (
                int(self._chaindrift.config["max_open_trades"])
                if self._chaindrift.config["max_open_trades"] != float("inf")
                else -1
            ),
    ...
    def _rpc_whitelist(self) -> dict:
        ...
        res = {
            "method": self._chaindrift.pairlists.name_list,
            "length": len(self._chaindrift.active_pair_whitelist),
            "whitelist": self._chaindrift.active_pair_whitelist,
        }
        return res
    ...
    def _rpc_blacklist_delete(self, delete: list[str]) -> dict:
        ...
        errors = {}
        for pair in delete:
            if pair in self._chaindrift.pairlists.blacklist:
                self._chaindrift.pairlists.blacklist.remove(pair)
            else:
    ...
    def _rpc_blacklist(self, add: list[str] | None = None) -> dict:
        ...
        if add:
            for pair in add:
                if pair not in self._chaindrift.pairlists.blacklist:
                    try:
                        expand_pairlist([pair], self._chaindrift.exchange.get_markets().keys())
                        self._chaindrift.pairlists.blacklist.append(pair)
        ...
        res = {
            "method": self._chaindrift.pairlists.name_list,
            "length": len(self._chaindrift.pairlists.blacklist),
            "blacklist": self._chaindrift.pairlists.blacklist,
            "blacklist_expanded": self._chaindrift.pairlists.expanded_blacklist,
            "errors": errors,
        }
        return res
    ...
    def _rpc_get_logs(limit: int | None) -> dict[str, Any]:
        ...
        #       "chaindrift.worker", "INFO", "Starting worker develop"]

        return {"log_count": len(records), "logs": records}

    def _rpc_edge(self) -> list[dict[str, Any]]:
        """Returns information related to Edge"""
        if not self._chaindrift.edge:
            raise RPCException("Edge is not enabled.")
        return self._chaindrift.edge.accepted_pairs()
    ...
    def _rpc_analysed_dataframe(
        ...
        return RPC._convert_dataframe_to_dict(
            self._chaindrift.config["strategy"], pair, timeframe, _data, last_analyzed, selected_cols
        )
    ...
    def __rpc_analysed_dataframe_raw(
        ...
        """
        _data, last_analyzed = self._chaindrift.dataprovider.get_analyzed_dataframe(pair, timeframe)
        _data = _data.copy()
    ...
    def _ws_all_analysed_dataframes(
        ...
        """
        timeframe = self._chaindrift.config["timeframe"]
        candle_type = self._chaindrift.config.get("candle_type_def", CandleType.SPOT)
    ...
    def _rpc_analysed_history_full(
        ...
        from chaindrift.data.converter import trim_dataframe
    ...
    def _rpc_plot_config(self) -> dict[str, Any]:
        if (
            self._chaindrift.strategy.plot_config
            and "subplots" not in self._chaindrift.strategy.plot_config
        ):
            self._chaindrift.strategy.plot_config["subplots"] = {}
        return self._chaindrift.strategy.plot_config
    ...
    def health(self) -> dict[str, str | int | None]:
        last_p = self._chaindrift.last_process
    ...
    def _update_market_direction(self, direction: MarketDirection) -> None:
        self._chaindrift.strategy.market_direction = direction

    def _get_market_direction(self) -> MarketDirection:
        return self._chaindrift.strategy.market_direction
