...
from chaindrift.rpc.api_server.api_schemas import (
    AvailablePairs,
    Balances,
    BlacklistPayload,
    BlacklistResponse,
...
from chaindrift.rpc.api_server.deps import get_config, get_exchange, get_rpc, get_rpc_optional
...
# 2.22: Add ChainAI to backtesting
...
def show_config(rpc: RPC | None = Depends(get_rpc_optional), config=Depends(get_config)):
    ...
    if rpc:
        state = rpc._chaindrift.state
        strategy_version = rpc._chaindrift.strategy.version()
    ...
    config.update(
        {
            "strategy": strategy,
            "timerange": timerange,
            "chainaimodel": chainaimodel if chainaimodel else config.get("chainaimodel"),
        }
    )
    try:
    ...
    config.update(
        {
            ...
            "chainaimodel": (
                payload.chainaimodel if payload.chainaimodel else config.get("chainaimodel")
            ),
        }
    )
    try:
...
def list_exchanges(config=Depends(get_config)):
    from chaindrift.exchange import list_available_exchanges
...
def list_hyperoptloss(
    ...
    from chaindrift.resolvers.hyperopt_resolver import HyperOptLossResolver
...
@router.get("/chainaimodels", response_model=ChainAIModelListResponse, tags=["chainai"])
def list_chainaimodels(config=Depends(get_config)):
    from chaindrift.resolvers.chainaimodel_resolver import ChainaiModelResolver

    models = ChainaiModelResolver.search_all_objects(config, False)
    models = sorted(models, key=lambda x: x["name"])

    return {"chainaimodels": [x["name"] for x in models]}
