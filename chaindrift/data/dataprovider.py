...
from chaindrift.data.history import get_datahandler, load_pair_history
from chaindrift.enums import CandleType, RPCMessageType, RunMode, TradingMode
from chaindrift.exceptions import ExchangeError, OperationalException
from chaindrift.exchange import Exchange, timeframe_to_prev_date, timeframe_to_seconds
from chaindrift.exchange.exchange_types import OrderBook
from chaindrift.misc import append_candles_to_dataframe
from chaindrift.rpc import RPCManager
from chaindrift.rpc.rpc_types import RPCAnalyzedDFMsg
from chaindrift.util import PeriodicCache
...
class DataProvider:
    ...
    def get_required_startup(self, timeframe: str) -> int:
        chainai_config = self._config.get("chainai", {})
        if not chainai_config.get("enabled", False):
            return self._config.get("startup_candle_count", 0)
        else:
            startup_candles = self._config.get("startup_candle_count", 0)
            indicator_periods = chainai_config["feature_parameters"]["indicator_periods_candles"]
            # make sure the startupcandles is at least the set maximum indicator periods
            self._config["startup_candle_count"] = max(startup_candles, max(indicator_periods))
            tf_seconds = timeframe_to_seconds(timeframe)
            train_candles = chainai_config["train_period_days"] * 86400 / tf_seconds
            total_candles = int(self._config["startup_candle_count"] + train_candles)
            logger.info(
                f"Increasing startup_candle_count for chainai on {timeframe} to {total_candles}"
            )
