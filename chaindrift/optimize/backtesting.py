...
from chaindrift.configuration import TimeRange, validate_config_consistency
from chaindrift.constants import DATETIME_PRINT_FORMAT, Config, IntOrInf, LongShort
from chaindrift.data import history
from chaindrift.data.btanalysis import find_existing_backtest_stats, trade_list_to_dataframe
from chaindrift.data.converter import trim_dataframe, trim_dataframes
from chaindrift.data.dataprovider import DataProvider
from chaindrift.data.metrics import combined_dataframes_with_rel_mean
...
from chaindrift.ft_types import BacktestResultType, get_BacktestResultType_default
from chaindrift.leverage.liquidation_price import update_liquidation_prices
from chaindrift.mixins import LoggingMixin
from chaindrift.optimize.backtest_caching import get_strategy_run_id
from chaindrift.optimize.bt_progress import BTProgress
from chaindrift.optimize.optimize_reports import (
...
from chaindrift.persistence import (
    CustomDataWrapper,
    LocalTrade,
    Order,
    PairLocks,
...
from chaindrift.plugins.protectionmanager import ProtectionManager
from chaindrift.resolvers import ExchangeResolver, StrategyResolver
...
from chaindrift.util import FtPrecise
from chaindrift.util.migrations import migrate_data
...
class Backtesting:
    ...
    def __init__(self, config: Config, exchange: Exchange | None = None) -> None:
        ...
        if self.config.get("strategy_list"):
            if self.config.get("chainai", {}).get("enabled", False):
                logger.warning(
                    "Using --strategy-list with ChainAI REQUIRES all strategies "
        ...
        if self.config.get("chainai_backtest_live_models", False):
            from chaindrift.chainai.utils import get_timerange_backtest_live_models
        ...
        if self.config.get("chainai", {}).get("enabled", False):
            # For ChainAI, increase the required_startup to includes the training data
    ...
    def _get_close_rate_for_roi(
        ...
        if roi is not None and roi_entry is not None:
            ...
            if trade_dur == 0 and (
                ...
                # details: https: // github.com/chaindrift/chaindrift/issues/6261
        ...
        if exit_.exit_flag:
            ...
            if exit_.exit_type in (
                ...
                if order_type == "limit":
                    ...
                    # chaindrift does not support this in live, and the order would fill immediately
                    if trade.is_short:
    ...
    def get_valid_price_and_stake(
        ...
        if order_type == "limit":
            ...
            # which chaindrift does not support in live.
