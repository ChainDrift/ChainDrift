...
from chaindrift.strategy import IntParameter, IStrategy, merge_informative_pair  # noqa


logger = logging.getLogger(__name__)


class ChainaiExampleHybridStrategy(IStrategy):
    """
    Example of a hybrid ChainAI strat, designed to illustrate how a user may employ
    ChainAI to bolster a typical Chaindrift strategy.

    Launching this strategy would be:

    chaindrift trade --strategy ChainaiExampleHybridStrategy --strategy-path chaindrift/templates
    --chainaimodel CatboostClassifier --config config_examples/config_chainai.example.json
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:  # noqa: C901
        ...
        # TA indicators to combine with the Chainai targets
        # RSI
    ...
    def populate_entry_trend(self, df: DataFrame, metadata: dict) -> DataFrame:
        df.loc[
            (
                ...
                & (df["do_predict"] == 1)  # Make sure Chainai is confident in the prediction
                &
                # Only enter trade if Chainai thinks the trend is in this direction
                (df["&s-up_or_down"] == "up")
            ),
            "enter_long",
        ] = 1
