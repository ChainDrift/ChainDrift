...
from chaindrift.constants import CUSTOM_TAG_MAX_LENGTH, Config, IntOrInf, ListPairsWithTimeframes
from chaindrift.data.converter import populate_dataframe_with_trades
...
from chaindrift.exceptions import OperationalException, StrategyError
from chaindrift.exchange import timeframe_to_minutes, timeframe_to_next_date, timeframe_to_seconds
from chaindrift.misc import remove_entry_exit_signals
from chaindrift.persistence import Order, PairLocks, Trade
from chaindrift.strategy.hyper import HyperStrategyMixin
from chaindrift.strategy.informative_decorator import (
    InformativeData,
    PopulateIndicators,
    _create_and_merge_informative_pair,
    _format_pair_name,
)
from chaindrift.strategy.strategy_wrapper import strategy_safe_wrapper
from chaindrift.util import dt_now
from chaindrift.wallets import Wallets
...
class IStrategy(ABC, HyperStrategyMixin):
    """
    Interface for chaindrift strategies
    ...
    def load_chainAI_model(self) -> None:
        if self.config.get("chainai", {}).get("enabled", False):
            # Import here to avoid importing this if chainAI is disabled
            from chaindrift.chainai.utils import download_all_data_for_training
            from chaindrift.resolvers.chainaimodel_resolver import ChainaiModelResolver

            self.chainai = ChainaiModelResolver.load_chainaimodel(self.config)
            self.chainai_info = self.config["chainai"]
            ...
            # Gracious failures if chainAI is disabled but "start" is called.
            class DummyClass:
                def start(self, *args, **kwargs):
                    raise OperationalException(
                        "chainAI is not enabled. "
            ...
            self.chainai = DummyClass()  # type: ignore
    ...
    def ft_bot_cleanup(self) -> None:
        """
        Clean up ChainAI and child threads
        """
        self.chainai.shutdown()
    ...
    def confirm_trade_entry(
        ...
        **kwargs,
        ...
        For full documentation please go to https://www.chaindrift.io/en/latest/strategy-advanced/
    ...
    def adjust_entry_price(
        ...
        **kwargs,
        ...
        For full documentation please go to https://www.chaindrift.io/en/latest/strategy-callbacks/
    ...
    def populate_any_indicators(
        ...
        This method is *only* used in ChainaiDataKitchen class and therefore
        it is only called if ChainAI is active.
    ...
    def __informative_pairs_chainai(self) -> ListPairsWithTimeframes:
        """
        Create informative-pairs needed for ChainAI
        """
        if self.config.get("chainai", {}).get("enabled", False):
            whitelist_pairs = self.dp.current_whitelist()
            candle_type = self.config.get("candle_type_def", CandleType.SPOT)
            corr_pairs = self.config["chainai"]["feature_parameters"]["include_corr_pairlist"]
            informative_pairs = []
            for tf in self.config["chainai"]["feature_parameters"]["include_timeframes"]:
    ...
    def gather_informative_pairs(self) -> ListPairsWithTimeframes:
        ...
        informative_pairs.extend(self.__informative_pairs_chainai())
    ...
    def unlock_pair(self, pair: str) -> None:
        ...
        Not used by chaindrift itself, but intended to be used if users lock pairs
