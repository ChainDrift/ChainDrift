...
$RequirementFiles = @("requirements.txt", "requirements-dev.txt", "requirements-hyperopt.txt", "requirements-chainai.txt", "requirements-chainai-rl.txt", "requirements-plot.txt")
$VenvName = ".venv"
...
function Main {
  ...
  # Install chaindrift from setup using the virtual environment's Python
  Write-Log "Installing chaindrift from setup..."
  pip install -e . 2>&1 | Out-File $LogFilePath -Append
  if ($LASTEXITCODE -ne 0) {
    Write-Log "Failed to install chaindrift." -Level 'ERROR'
    Exit-Script -exitCode 1
  }

  Write-Log "Installing chainUI..."
  python chaindrift install-ui 2>&1 | Out-File $LogFilePath -Append
  if ($LASTEXITCODE -ne 0) {
    Write-Log "Failed to install chainUI." -Level 'ERROR'
    Exit-Script -exitCode 1
  }
