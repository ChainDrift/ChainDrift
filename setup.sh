...
function updateenv() {
    ...
    REQUIREMENTS_PLOT=""
    REQUIREMENTS_CHAINAI=""
    REQUIREMENTS_CHAINAI_RL=""
    ...
    then
        ...
        read -p "Do you want to install dependencies for chainai [y/N]? "
        if [[ $REPLY =~ ^[Yy]$ ]]
        then
            REQUIREMENTS_CHAINAI="-r requirements-chainai.txt --use-pep517"
            read -p "Do you also want dependencies for chainai-rl or PyTorch (~700mb additional space required) [y/N]? "
            if [[ $REPLY =~ ^[Yy]$ ]]
            then
                REQUIREMENTS_CHAINAI="-r requirements-chainai-rl.txt"
            fi
        fi
    fi
    install_talib

    ${PYTHON} -m pip install --upgrade -r ${REQUIREMENTS} ${REQUIREMENTS_HYPEROPT} ${REQUIREMENTS_PLOT} ${REQUIREMENTS_CHAINAI} ${REQUIREMENTS_CHAINAI_RL}
    if [ $? -ne 0 ]; then
    ...
    if [ $? -ne 0 ]; then
        echo "Failed installing Chaindrift"
        exit 1
    fi

    echo "Installing chainUI"
    chaindrift install-ui
...
function config() {
    echo_block "Please use 'chaindrift new-config -c user_data/config.json' to generate a new configuration file."
}
...
function install() {
    ...
    echo "You can now use the bot by executing 'source .venv/bin/activate; chaindrift <subcommand>'."
    echo "You can see the list of available bot sub-commands by executing 'source .venv/bin/activate; chaindrift --help'."
    echo "You verify that chaindrift is installed successfully by running 'source .venv/bin/activate; chaindrift --version'."
}
...
function help() {
    echo "usage:"
    echo "	-i,--install    Install chaindrift from scratch"
