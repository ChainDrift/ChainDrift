...
from chaindrift.constants import AVAILABLE_PAIRLISTS
from chaindrift.data.dataprovider import DataProvider
from chaindrift.enums import CandleType, RunMode
from chaindrift.exceptions import OperationalException
from chaindrift.persistence import LocalTrade, Trade
from chaindrift.plugins.pairlist.pairlist_helpers import dynamic_expand_pairlist, expand_pairlist
from chaindrift.plugins.pairlistmanager import PairListManager
from chaindrift.resolvers import PairListResolver
...
def test_log_cached(mocker, static_pl_conf, markets, tickers):
    ...
    chaindrift = get_patched_chaindriftbot(mocker, static_pl_conf)
    logmock = MagicMock()
    # Assign starting whitelist
    pl = chaindrift.pairlists._pairlist_handlers[0]
...
def test_load_pairlist_noexist(mocker, markets, default_conf):
    ...
    plm = PairListManager(chaindrift.exchange, default_conf, MagicMock())
    with pytest.raises(
        ...
        PairListResolver.load_pairlist(
            "NonexistingPairList", chaindrift.exchange, plm, default_conf, {}, 1
        )
...
def test_refresh_market_pair_not_in_whitelist(mocker, markets, static_pl_conf):
    ...
    mocker.patch(f"{EXMS}.markets", PropertyMock(return_value=markets))
    chaindrift.pairlists.refresh_pairlist()
    # List ordered by BaseVolume
    whitelist = ["ETH/BTC", "TKN/BTC"]
    # Ensure all except those in whitelist are removed
    assert set(whitelist) == set(chaindrift.pairlists.whitelist)
    # Ensure config dict hasn't been changed
    assert (
        static_pl_conf["exchange"]["pair_whitelist"]
        == chaindrift.config["exchange"]["pair_whitelist"]
    )
...
def test_refresh_static_pairlist(mocker, markets, static_pl_conf):
    ...
    # Ensure all except those in whitelist are removed
    assert set(whitelist) == set(chaindrift.pairlists.whitelist)
    assert static_pl_conf["exchange"]["pair_blacklist"] == chaindrift.pairlists.blacklist
...
def test_refresh_static_pairlist_noexist(mocker, markets, static_pl_conf, pairs, expected, caplog):
    ...
    # Ensure all except those in whitelist are removed
    assert set(expected) == set(chaindrift.pairlists.whitelist)
    assert static_pl_conf["exchange"]["pair_blacklist"] == chaindrift.pairlists.blacklist
    if not expected:
...
def test_remove_logs_for_pairs_already_in_blacklist(mocker, markets, static_pl_conf, caplog):
    ...
    for _ in range(3):
        new_whitelist = chaindrift.pairlists.verify_blacklist(
...
def test_refresh_pairlist_dynamic(mocker, shitcoinmarkets, tickers, whitelist_conf):
    ...
    chaindrift = get_patched_chaindriftbot(mocker, whitelist_conf)
    # Remock markets with shitcoinmarkets since get_patched_chaindriftbot uses the markets fixture
    mocker.patch.multiple(
        EXMS,
    ...
    chaindrift.pairlists.refresh_pairlist()
    assert whitelist == chaindrift.pairlists.whitelist
    ...
    with pytest.raises(
        ...
        PairListManager(chaindrift.exchange, whitelist_conf, MagicMock())
...
def test_refresh_pairlist_dynamic_2(mocker, shitcoinmarkets, tickers, whitelist_conf_2):
    ...
    mocker.patch.multiple(
        "chaindrift.plugins.pairlistmanager.PairListManager",
        _get_cached_tickers=MagicMock(return_value=tickers_dict),
    )
    chaindrift = get_patched_chaindriftbot(mocker, whitelist_conf_2)
    # Remock markets with shitcoinmarkets since get_patched_chaindriftbot uses the markets fixture
    mocker.patch.multiple(
        EXMS,
        markets=PropertyMock(return_value=shitcoinmarkets),
...
def test_VolumePairList_whitelist_gen(
    ...
    mocker.patch.multiple(
        "chaindrift.persistence.Trade", get_overall_performance=MagicMock(return_value=[])
    )
    ...
    if whitelist_result == "filter_at_the_beginning":
        ...
        chaindrift.pairlists.refresh_pairlist()
        whitelist = chaindrift.pairlists.whitelist
...
def test_gen_pair_whitelist_not_supported(mocker, default_conf, tickers) -> None:
    ...
    with pytest.raises(
        ...
        get_patched_chaindriftbot(mocker, default_conf)
...
def test_pairlist_class(mocker, whitelist_conf, markets, pairlist):
    ...
    assert chaindrift.pairlists.name_list == [pairlist]
    assert pairlist in str(chaindrift.pairlists.short_desc())
    assert isinstance(chaindrift.pairlists.whitelist, list)
    assert isinstance(chaindrift.pairlists.blacklist, list)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (["ETH/BTC", "TKN/BTC", "XLTCUSDT"], "is not tradable with Chaindrift"),
    ],
)
def test__whitelist_for_active_markets(
    ...
    # Assign starting whitelist
    pairlist_handler = chaindrift.pairlists._pairlist_handlers[0]
...
def test_volumepairlist_invalid_sortvalue(mocker, whitelist_conf):
    ...
    with pytest.raises(OperationalException, match=r"key asdf not in .*"):
        get_patched_chaindriftbot(mocker, whitelist_conf)
...
def test_volumepairlist_caching(mocker, markets, whitelist_conf, tickers):
    ...
    chaindrift = get_patched_chaindriftbot(mocker, whitelist_conf)
    assert len(chaindrift.pairlists._pairlist_handlers[0]._pair_cache) == 0
    assert tickers.call_count == 0
    chaindrift.pairlists.refresh_pairlist()
    assert tickers.call_count == 1

    assert len(chaindrift.pairlists._pairlist_handlers[0]._pair_cache) == 1
    chaindrift.pairlists.refresh_pairlist()
    assert tickers.call_count == 1
...
def test_agefilter_caching(mocker, markets, whitelist_conf_agefilter, tickers, ohlcv_history):
    with time_machine.travel("2021-09-01 05:00:00 +00:00") as t:
        ...
        chaindrift = get_patched_chaindriftbot(mocker, whitelist_conf_agefilter)
        assert chaindrift.exchange.refresh_latest_ohlcv.call_count == 0
        chaindrift.pairlists.refresh_pairlist()
        assert len(chaindrift.pairlists.whitelist) == 3
        assert chaindrift.exchange.refresh_latest_ohlcv.call_count > 0

        chaindrift.pairlists.refresh_pairlist()
        assert len(chaindrift.pairlists.whitelist) == 3
        # Call to XRP/BTC cached
        assert chaindrift.exchange.refresh_latest_ohlcv.call_count == 2
        ...
        chaindrift.pairlists.refresh_pairlist()
        assert len(chaindrift.pairlists.whitelist) == 3
        assert chaindrift.exchange.refresh_latest_ohlcv.call_count == 1
        ...
        mocker.patch(f"{EXMS}.refresh_latest_ohlcv", return_value=ohlcv_data)
        chaindrift.pairlists.refresh_pairlist()
        assert len(chaindrift.pairlists.whitelist) == 4
...
def test_rangestabilityfilter_caching(
    ...
    assert chaindrift.exchange.refresh_latest_ohlcv.call_count == 0
    chaindrift.pairlists.refresh_pairlist()
    assert len(chaindrift.pairlists.whitelist) == expected_length
    assert chaindrift.exchange.refresh_latest_ohlcv.call_count > 0

    previous_call_count = chaindrift.exchange.refresh_latest_ohlcv.call_count
    chaindrift.pairlists.refresh_pairlist()
    assert len(chaindrift.pairlists.whitelist) == expected_length
    # Should not have increased since first call.
    assert chaindrift.exchange.refresh_latest_ohlcv.call_count == previous_call_count
...
def test_pricefilter_desc(
    ...
    if desc_expected is not None:
        chaindrift = get_patched_chaindriftbot(mocker, whitelist_conf)
        short_desc = str(chaindrift.pairlists.short_desc())
...
def test_performance_filter(
    ...
    chaindrift = get_patched_chaindriftbot(mocker, allowlist_conf)
    ...
    mocker.patch.multiple(
        "chaindrift.persistence.Trade",
        get_overall_performance=MagicMock(return_value=overall_performance),
    )
    chaindrift.pairlists.refresh_pairlist()
    allowlist = chaindrift.pairlists.whitelist
...
def test_MarketCapPairList_filter(
    ...
    mocker.patch(
        "chaindrift.plugins.pairlist.MarketCapPairList.FtCoinGeckoApi.get_coins_categories_list",
        return_value=[
            {"category_id": "layer-1"},
    ...
    gcm_mock = mocker.patch(
        "chaindrift.plugins.pairlist.MarketCapPairList.FtCoinGeckoApi.get_coins_markets",
