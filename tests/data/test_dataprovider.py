...
from chaindrift.exceptions import ExchangeError, OperationalException
...
def test_historic_ohlcv(mocker, default_conf, ohlcv_history):
    ...
    mocker.patch("chaindrift.data.dataprovider.load_pair_history", historymock)
...
def test_historic_trades(mocker, default_conf, trades_history_df):
    ...
    mocker.patch(
        "chaindrift.data.history.datahandlers.featherdatahandler.FeatherDataHandler._trades_load",
        historymock,
    )
...
def test_historic_ohlcv_dataformat(mocker, default_conf, ohlcv_history):
    ...
    mocker.patch(
        "chaindrift.data.history.datahandlers.hdf5datahandler.HDF5DataHandler._ohlcv_load",
        hdf5loadmock,
    )
    mocker.patch(
...
def test_emit_df(mocker, default_conf, ohlcv_history):
    mocker.patch("chaindrift.rpc.rpc_manager.RPCManager.__init__", MagicMock())
    rpc_mock = mocker.patch("chaindrift.rpc.rpc_manager.RPCManager", MagicMock())
    send_mock = mocker.patch("chaindrift.rpc.rpc_manager.RPCManager.send_msg", MagicMock())
...
def test_dp_get_required_startup(default_conf_usdt):
    ...
    # ChainAI kindof ignores startup_candle_count if it's below indicator_periods_candles
    dp._config["startup_candle_count"] = 0
    assert dp.get_required_startup("5m") == 5780
    assert dp.get_required_startup("1h") == 500
    assert dp.get_required_startup("1d") == 40

    dp._config["chainai"]["feature_parameters"]["indicator_periods_candles"][1] = 50
    assert dp.get_required_startup("5m") == 5810
    assert dp.get_required_startup("1h") == 530
    assert dp.get_required_startup("1d") == 70

    # scenario from issue https://github.com/chaindrift/chaindrift/issues/9432
    dp._config["chainai"] = {
        "enabled": True,
