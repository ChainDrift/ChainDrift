...
from chaindrift.commands import (
...
from chaindrift.commands.deploy_ui import (
    clean_ui_subdir,
...
from chaindrift.util import dt_utc
...
def test_start_trading_fail(mocker, caplog):
    mocker.patch("chaindrift.worker.Worker.run", MagicMock(side_effect=OperationalException))

    mocker.patch("chaindrift.worker.Worker.__init__", MagicMock(return_value=None))

    exitmock = mocker.patch("chaindrift.worker.Worker.exit", MagicMock())
    ...
    exitmock.reset_mock()
    caplog.clear()
    mocker.patch("chaindrift.worker.Worker.__init__", MagicMock(side_effect=OperationalException))
    with pytest.raises(OperationalException):
        start_trading(get_args(args))
...
def test_start_webserver(mocker, caplog):
    api_server_mock = mocker.patch(
        "chaindrift.rpc.api_server.ApiServer",
    )
...
def test_list_exchanges(capsys):
    ...
    assert re.search(r".*Exchanges available for Chaindrift.*", captured.out)
...
def test_create_datadir(mocker):
    cud = mocker.patch(
        "chaindrift.configuration.directory_operations.create_userdata_dir", MagicMock()
    )
    csf = mocker.patch(
        "chaindrift.configuration.directory_operations.copy_sample_files", MagicMock()
    )
    args = ["create-userdir", "--userdir", "/temp/chaindrift/test"]
    start_create_userdir(get_args(args))
...
def test_start_install_ui(mocker):
    clean_mock = mocker.patch("chaindrift.commands.deploy_ui.clean_ui_subdir")
    get_url_mock = mocker.patch(
        "chaindrift.commands.deploy_ui.get_ui_download_url",
        return_value=("https://example.com/whatever", "0.0.1"),
    )
    download_mock = mocker.patch("chaindrift.commands.deploy_ui.download_and_install_ui")
    mocker.patch("chaindrift.commands.deploy_ui.read_ui_version", return_value=None)
    args = [
        "install-ui",
    ]
...
def test_clean_ui_subdir(mocker, tmp_path, caplog):
    mocker.patch("chaindrift.commands.deploy_ui.Path.is_dir", side_effect=[True, True])
    mocker.patch("chaindrift.commands.deploy_ui.Path.is_file", side_effect=[False, True])
    rd_mock = mocker.patch("chaindrift.commands.deploy_ui.Path.rmdir")
    ul_mock = mocker.patch("chaindrift.commands.deploy_ui.Path.unlink")

    mocker.patch(
        "chaindrift.commands.deploy_ui.Path.glob",
...
def test_download_and_install_ui(mocker, tmp_path):
    ...
    mocker.patch("chaindrift.commands.deploy_ui.requests.get", return_value=requests_mock)

    mocker.patch("chaindrift.commands.deploy_ui.Path.is_dir", side_effect=[True, False])
    wb_mock = mocker.patch("chaindrift.commands.deploy_ui.Path.write_bytes")
...
def test_get_ui_download_url(mocker):
    ...
    get_mock = mocker.patch("chaindrift.commands.deploy_ui.requests.get", return_value=response)
...
def test_download_data_keyboardInterrupt(mocker, markets):
    dl_mock = mocker.patch(
        "chaindrift.data.history.download_data_main",
...
def test_download_data_timerange(mocker, markets, time_machine, time, tzoffset):
    ...
    dl_mock = mocker.patch(
        "chaindrift.data.history.history_utils.refresh_backtest_ohlcv_data",
...
def test_download_data_trades(mocker):
    dl_mock = mocker.patch(
        "chaindrift.data.history.history_utils.refresh_backtest_trades_data",
        MagicMock(return_value=[]),
    )
    convert_mock = mocker.patch(
        "chaindrift.data.history.history_utils.convert_trades_to_ohlcv", MagicMock(return_value=[])
    )
    patch_exchange(mocker)
...
def test_start_convert_trades(mocker):
    convert_mock = mocker.patch(
        "chaindrift.data.converter.convert_trades_to_ohlcv", MagicMock(return_value=[])
    )
    patch_exchange(mocker)
...
def test_start_list_chainAI_models(capsys):
    args = ["list-chainaimodels", "-1"]
    pargs = get_args(args)
    pargs["config"] = None
    start_list_chainAI_models(pargs)
    captured = capsys.readouterr()
...
def test_hyperopt_list(mocker, capsys, caplog, tmp_path):
    ...
    mocker.patch(
        "chaindrift.optimize.hyperopt_tools.HyperoptTools._test_hyperopt_results_exist",
        return_value=True,
    )
    ...
    mocker.patch(
        "chaindrift.optimize.hyperopt_tools.HyperoptTools._read_results", side_effect=fake_iterator
    )
...
def test_hyperopt_show(mocker, capsys):
    ...
    mocker.patch("chaindrift.optimize.optimize_reports.show_backtest_result")
...
def test_convert_data(mocker, testdatadir):
    ohlcv_mock = mocker.patch("chaindrift.data.converter.convert_ohlcv_format")
    trades_mock = mocker.patch("chaindrift.data.converter.convert_trades_format")
    args = [
        "convert-data",
        "--format-from",
        "json",
...
def test_show_trades(mocker, fee, capsys, caplog):
    mocker.patch("chaindrift.persistence.init_db")
...
def test_backtesting_show(mocker, testdatadir, capsys):
    sbr = mocker.patch("chaindrift.optimize.optimize_reports.show_backtest_results")
    args = [
        "backtesting-show",
        "--export-filename",
...
def test_start_strategy_updater(mocker, tmp_path):
    sc_mock = mocker.patch("chaindrift.commands.strategy_utils_commands.start_conversion")
