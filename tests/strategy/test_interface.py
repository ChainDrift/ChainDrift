...
from chaindrift.constants import CUSTOM_TAG_MAX_LENGTH
from chaindrift.data.dataprovider import DataProvider
from chaindrift.data.history import load_data
from chaindrift.enums import ExitCheckTuple, ExitType, HyperoptState, SignalDirection
from chaindrift.exceptions import OperationalException, StrategyError
from chaindrift.optimize.hyperopt_tools import HyperoptStateContainer
...
from chaindrift.strategy.hyper import detect_parameters
...
def test_chainai_not_initialized(default_conf) -> None:
    strategy = StrategyResolver.load_strategy(default_conf)
    strategy.ft_bot_start()
    with pytest.raises(OperationalException, match=r"chainAI is not enabled\."):
        strategy.chainai.start()


def test_advise_all_indicators_copy(mocker, default_conf, testdatadir) -> None:
    strategy = StrategyResolver.load_strategy(default_conf)
    aimock = mocker.patch("chaindrift.strategy.interface.IStrategy.advise_indicators")
    timerange = TimeRange.parse_timerange("1510694220-1510700340")
    data = load_data(testdatadir, "1m", ["UNITTEST/BTC"], timerange=timerange, fill_up_missing=True)
...
def test_auto_hyperopt_interface_loadparams(default_conf, mocker, caplog):
    ...
    mocker.patch("chaindrift.strategy.hyper.HyperoptTools.load_params", return_value=expected_result)
    PairLocks.timeframe = default_conf["timeframe"]
    strategy = StrategyResolver.load_strategy(default_conf)
    ...
    mocker.patch(
        "chaindrift.strategy.hyper.HyperoptTools.load_params", MagicMock(side_effect=ValueError())
    )
