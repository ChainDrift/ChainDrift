...
from chaindrift.loggers import (
    FTBufferingHandler,
...
from chaindrift.loggers.set_log_levels import (
...
def test_reduce_verbosity():
    setup_logging_pre()
    reduce_verbosity_for_bias_tester()
    prior_level = logging.getLogger("chaindrift").getEffectiveLevel()

    assert logging.getLogger("chaindrift.resolvers").getEffectiveLevel() == logging.WARNING
    assert logging.getLogger("chaindrift.strategy.hyper").getEffectiveLevel() == logging.WARNING
    # base level wasn't changed
    assert logging.getLogger("chaindrift").getEffectiveLevel() == prior_level

    restore_verbosity_for_bias_tester()

    assert logging.getLogger("chaindrift.resolvers").getEffectiveLevel() == prior_level
    assert logging.getLogger("chaindrift.strategy.hyper").getEffectiveLevel() == prior_level
    assert logging.getLogger("chaindrift").getEffectiveLevel() == prior_level
    # base level wasn't changed
