...
from chaindrift.commands.optimize_commands import start_lookahead_analysis
from chaindrift.data.history import get_timerange
from chaindrift.exceptions import OperationalException
from chaindrift.optimize.analysis.lookahead import Analysis, LookaheadAnalysis
...
IGNORE_BIASED_INDICATORS_CAPTION = (
    ...
    "set_chainai_targets() can be ignored."
)
...
def test_start_lookahead_analysis(mocker):
    ...
    mocker.patch.multiple(
        "chaindrift.optimize.analysis.lookahead_helpers.LookaheadAnalysisSubFunctions",
...
def test_lookahead_helper_text_table_lookahead_analysis_instances__caption(
    ...
    mocker.patch(
        "chaindrift.optimize.analysis.lookahead_helpers.print_rich_table",
...
def test_initialize_single_lookahead_analysis(lookahead_conf, mocker, caplog):
    ...
    start_mock = mocker.patch("chaindrift.optimize.analysis.lookahead.LookaheadAnalysis.start")
    strategy_obj = {
        "name": "strategy_test_v3_with_lookahead_bias",
        "location": Path(lookahead_conf["strategy_path"], f"{lookahead_conf['strategy']}.py"),
    }
