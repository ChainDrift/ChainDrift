...
from chaindrift.commands.optimize_commands import start_recursive_analysis
from chaindrift.data.history import get_timerange
from chaindrift.exceptions import OperationalException
from chaindrift.optimize.analysis.recursive import RecursiveAnalysis
...
def test_start_recursive_analysis(mocker):
    ...
    mocker.patch.multiple(
        "chaindrift.optimize.analysis.recursive_helpers.RecursiveAnalysisSubFunctions",
...
def test_initialize_single_recursive_analysis(recursive_conf, mocker, caplog):
    ...
    start_mock = mocker.patch("chaindrift.optimize.analysis.recursive.RecursiveAnalysis.start")
    strategy_obj = {
        "name": "strategy_test_v3_recursive_issue",
        "location": Path(recursive_conf["strategy_path"], f"{recursive_conf['strategy']}.py"),
    }
