...
from chaindrift import constants
from chaindrift.commands.optimize_commands import setup_optimize_configuration, start_backtesting
from chaindrift.configuration import TimeRange
from chaindrift.data import history
from chaindrift.data.btanalysis import BT_DATA_COLUMNS, evaluate_result_multi
from chaindrift.data.converter import clean_ohlcv_dataframe, ohlcv_fill_up_missing_data
from chaindrift.data.dataprovider import DataProvider
from chaindrift.data.history import get_timerange
from chaindrift.enums import CandleType, ExitType, RunMode
from chaindrift.exceptions import DependencyException, OperationalException
from chaindrift.exchange import timeframe_to_next_date, timeframe_to_prev_date
from chaindrift.optimize.backtest_caching import get_backtest_metadata_filename, get_strategy_run_id
...
from chaindrift.util.datetime_helpers import dt_utc
...
def test_start(mocker, fee, default_conf, caplog) -> None:
    ...
    patch_exchange(mocker)
    mocker.patch("chaindrift.optimize.backtesting.Backtesting.start", start_mock)
    patched_configuration_load_config_file(mocker, default_conf)
    ...
    assert log_has("Starting chaindrift in Backtesting mode", caplog)
...
def test_backtesting_start(default_conf, mocker, caplog) -> None:
    ...
    mocker.patch("chaindrift.data.history.get_timerange", get_timerange)
    patch_exchange(mocker)
    mocker.patch("chaindrift.optimize.backtesting.Backtesting.backtest")
    mocker.patch("chaindrift.optimize.backtesting.generate_backtest_stats")
    mocker.patch("chaindrift.optimize.backtesting.show_backtest_results")
    sbs = mocker.patch("chaindrift.optimize.backtesting.store_backtest_stats")
    sbc = mocker.patch("chaindrift.optimize.backtesting.store_backtest_analysis_results")
    mocker.patch(
...
def test_backtesting_start_no_data(default_conf, mocker, caplog, testdatadir) -> None:
    ...
    mocker.patch(
        "chaindrift.data.history.history_utils.load_pair_history",
...
def test_backtesting_no_pair_left(default_conf, mocker) -> None:
    ...
    mocker.patch("chaindrift.optimize.backtesting.Backtesting.backtest")
    mocker.patch(
        "chaindrift.plugins.pairlistmanager.PairListManager.whitelist", PropertyMock(return_value=[])
    )
...
def test_backtesting_pairlist_list(default_conf, mocker, tickers) -> None:
    ...
    mocker.patch("chaindrift.plugins.pairlistmanager.PairListManager.refresh_pairlist")
...
def test_backtest__enter_trade_futures(default_conf_usdt, fee, mocker) -> None:
    ...
    mocker.patch("chaindrift.optimize.backtesting.price_to_precision", lambda p, *args: p)
    patch_exchange(mocker)
    ...
    backtesting._set_strategy(backtesting.strategylist[0])
    mocker.patch("chaindrift.optimize.backtesting.Backtesting._run_funding_fees")
    pair = "ETH/USDT:USDT"
    row = [
        pd.Timestamp(year=2020, month=1, day=1, hour=5, minute=0),
        0.1,  # Open
    ...
    # chaindrift_liquidation_price = liq + (abs(open_rate - liq) * liq_buffer * side_1)
    ...
    mocker.patch(
        "chaindrift.wallets.Wallets.get_trade_stake_amount", side_effect=DependencyException
    )
...
def test_backtest_start_multi_strat(default_conf, mocker, caplog, testdatadir):
    ...
    mocker.patch("chaindrift.optimize.backtesting.Backtesting.backtest", backtestmock)
    ...
    mocker.patch.multiple(
        "chaindrift.optimize.optimize_reports.bt_output",
        text_table_bt_results=text_table_mock,
        text_table_strategy=strattable_mock,
    )
    mocker.patch.multiple(
        "chaindrift.optimize.optimize_reports.optimize_reports",
...
def test_backtest_start_multi_strat_caching(
    ...
    mocker.patch("chaindrift.optimize.backtesting.Backtesting.backtest", backtestmock)
    mocker.patch("chaindrift.optimize.backtesting.show_backtest_results", MagicMock())
    ...
    mocker.patch.multiple(
        "chaindrift.data.btanalysis",
        load_backtest_metadata=load_backtest_metadata,
        load_backtest_stats=load_backtest_stats,
    )
    mocker.patch("chaindrift.optimize.backtesting.get_strategy_run_id", side_effect=["1", "2", "2"])
