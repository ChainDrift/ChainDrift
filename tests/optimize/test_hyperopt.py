...
from chaindrift.commands.optimize_commands import setup_optimize_configuration, start_hyperopt
from chaindrift.data.history import load_data
from chaindrift.enums import ExitType, RunMode
from chaindrift.exceptions import OperationalException
from chaindrift.optimize.hyperopt import Hyperopt
from chaindrift.optimize.hyperopt.hyperopt_auto import HyperOptAuto
from chaindrift.optimize.hyperopt_tools import HyperoptTools
from chaindrift.optimize.optimize_reports import generate_strategy_stats
from chaindrift.optimize.space import SKDecimal
from chaindrift.strategy import IntParameter
...
def test_setup_hyperopt_configuration_with_arguments(mocker, default_conf, caplog) -> None:
    patched_configuration_load_config_file(mocker, default_conf)
    mocker.patch("chaindrift.configuration.configuration.create_datadir", lambda c, x: x)
...
def test_start_not_installed(mocker, default_conf, import_fails) -> None:
    ...
    mocker.patch("chaindrift.optimize.hyperopt.Hyperopt.start", start_mock)
...
def test_start_no_data(mocker, hyperopt_conf, tmp_path) -> None:
    ...
    patched_configuration_load_config_file(mocker, hyperopt_conf)
    mocker.patch("chaindrift.data.history.load_pair_history", MagicMock(return_value=pd.DataFrame))
    mocker.patch(
        "chaindrift.optimize.hyperopt.hyperopt_optimizer.get_timerange",
...
def test_start_filelock(mocker, hyperopt_conf, caplog) -> None:
    ...
    patched_configuration_load_config_file(mocker, hyperopt_conf)
    mocker.patch("chaindrift.optimize.hyperopt.Hyperopt.__init__", hyperopt_mock)
    patch_exchange(mocker)
    ...
    pargs = get_args(args)
    start_hyperopt(pargs)
    assert log_has("Another running instance of chaindrift Hyperopt detected.", caplog)
...
def test_start_calls_optimizer(mocker, hyperopt_conf, capsys) -> None:
    dumper = mocker.patch("chaindrift.optimize.hyperopt.hyperopt_optimizer.dump")
    dumper2 = mocker.patch("chaindrift.optimize.hyperopt.Hyperopt._save_result")
    mocker.patch(
        "chaindrift.optimize.hyperopt.hyperopt_optimizer.calculate_market_change", return_value=1.5
    )
    mocker.patch("chaindrift.optimize.hyperopt.hyperopt.file_dump_json")

    mocker.patch(
        "chaindrift.optimize.backtesting.Backtesting.load_bt_data",
    ...
    mocker.patch("chaindrift.optimize.hyperopt.hyperopt.INITIAL_POINTS", 2)

    parallel = mocker.patch(
        "chaindrift.optimize.hyperopt.Hyperopt.run_optimizer_parallel",
        MagicMock(
            return_value=[
                {
                    "loss": 1,
...
def test_generate_optimizer(mocker, hyperopt_conf) -> None:
    ...
    mocker.patch(
        "chaindrift.optimize.hyperopt.hyperopt_optimizer.Backtesting.backtest",
    ...
    mocker.patch("chaindrift.configuration.config_validation.validate_config_schema")
    mocker.patch(
        "chaindrift.optimize.hyperopt.hyperopt_optimizer.load", return_value={"XRP/BTC": None}
    )
...
def test_clean_hyperopt(mocker, hyperopt_conf, caplog):
    ...
    mocker.patch(
        "chaindrift.strategy.hyper.HyperStrategyMixin.load_params_from_file",
        MagicMock(return_value={}),
    )
    mocker.patch("chaindrift.optimize.hyperopt.hyperopt.Path.is_file", MagicMock(return_value=True))
    unlinkmock = mocker.patch("chaindrift.optimize.hyperopt.hyperopt.Path.unlink", MagicMock())
...
def test_simplified_interface_all_failed(mocker, hyperopt_conf, caplog) -> None:
    mocker.patch("chaindrift.optimize.hyperopt.hyperopt_optimizer.dump", MagicMock())
    mocker.patch("chaindrift.optimize.hyperopt.hyperopt.file_dump_json")
    mocker.patch(
        "chaindrift.optimize.backtesting.Backtesting.load_bt_data",
        MagicMock(return_value=(MagicMock(), None)),
    ...
    mocker.patch(
        "chaindrift.optimize.hyperopt.hyperopt_auto.HyperOptAuto._generate_indicator_space",
        return_value=[],
    )
...
def test_in_strategy_auto_hyperopt_per_epoch(mocker, hyperopt_conf, tmp_path, fee) -> None:
    ...
    go = mocker.patch(
        "chaindrift.optimize.hyperopt.hyperopt_optimizer.HyperOptimizer.generate_optimizer",
        return_value={
            "loss": 0.05,
            "results_explanation": "foo result",
            "params": {},
...
def test_stake_amount_unlimited_max_open_trades(mocker, hyperopt_conf, tmp_path, fee) -> None:
    ...
    hyperopt = Hyperopt(hyperopt_conf)
    mocker.patch(
        "chaindrift.optimize.hyperopt.hyperopt_optimizer.HyperOptimizer._get_params_dict",
