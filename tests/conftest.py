...
from chaindrift.data.converter import ohlcv_to_dataframe, trades_list_to_df
from chaindrift.edge import PairInfo
from chaindrift.enums import CandleType, MarginMode, RunMode, SignalDirection, TradingMode
from chaindrift.exchange import Exchange, timeframe_to_minutes, timeframe_to_seconds
from chaindrift.chaindriftbot import ChaindriftBot
from chaindrift.persistence import LocalTrade, Order, Trade, init_db
from chaindrift.resolvers import ExchangeResolver
from chaindrift.util import dt_now, dt_ts
...
EXMS = "chaindrift.exchange.exchange.Exchange"
...
def patched_configuration_load_config_file(mocker, config) -> None:
    mocker.patch(
        "chaindrift.configuration.load_config.load_config_file", lambda *args, **kwargs: config
    )
...
def patch_exchange(
    ...
    # Temporary patch ...
    mocker.patch("chaindrift.exchange.bybit.Bybit.cache_leverage_tiers")
    ...
    if mock_supported_modes:
        mocker.patch(
            f"chaindrift.exchange.{exchange}.{exchange.capitalize()}"
...
def patch_wallet(mocker, free=999.9) -> None:
    mocker.patch("chaindrift.wallets.Wallets.get_free", MagicMock(return_value=free))


def patch_whitelist(mocker, conf) -> None:
    mocker.patch(
        "chaindrift.chaindriftbot.ChaindriftBot._refresh_active_whitelist",
...
def patch_edge(mocker) -> None:
    ...
    mocker.patch("chaindrift.edge.Edge.calculate", MagicMock(return_value=True))


# Functions for recurrent object patching


def patch_chaindriftbot(mocker, config) -> None:
    """
    ...
    mocker.patch("chaindrift.chaindriftbot.RPCManager._init", MagicMock())
    mocker.patch("chaindrift.chaindriftbot.RPCManager.send_msg", MagicMock())
    patch_whitelist(mocker, config)
    mocker.patch("chaindrift.chaindriftbot.ExternalMessageConsumer")
    mocker.patch("chaindrift.configuration.config_validation._validate_consumers")


def get_patched_chaindriftbot(mocker, config) -> ChaindriftBot:
    """
    This function patches _init_modules() to not call dependencies
    :param mocker: a Mocker object to apply patches
    :param config: Config to pass to the bot
    :return: ChaindriftBot
    """
    patch_chaindriftbot(mocker, config)
    return ChaindriftBot(config)
...
def patch_get_signal(
    ...
    chaindrift.strategy.get_entry_signal = patched_get_entry_signal
    ...
    # returns (enter, exit)
    chaindrift.strategy.get_exit_signal = patched_get_exit_signal

    chaindrift.exchange.refresh_latest_ohlcv = lambda p: None
...
def patch_gc(mocker) -> None:
    mocker.patch("chaindrift.main.gc_set_threshold")


@pytest.fixture(autouse=True)
def user_dir(mocker, tmp_path) -> Path:
    user_dir = tmp_path / "user_data"
    mocker.patch("chaindrift.configuration.configuration.create_userdata_dir", return_value=user_dir)
    return user_dir
...
def patch_coingecko(mocker) -> None:
    ...
    mocker.patch.multiple(
        "chaindrift.rpc.fiat_convert.FtCoinGeckoApi",
