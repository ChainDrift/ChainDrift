...
from chaindrift.enums import ExitCheckTuple, ExitType, RPCMessageType
from chaindrift.exceptions import ExchangeError, InsufficientFundsError, InvalidOrderException
from chaindrift.chaindriftbot import ChaindriftBot
from chaindrift.persistence import Order, Trade
...
from tests.chaindriftbot.test_chaindriftbot import patch_RPCManager
...
def test_add_stoploss_on_exchange(mocker, default_conf_usdt, limit_order, is_short, fee) -> None:
    ...
    chaindrift.exit_positions(trades)
...
def test_handle_stoploss_on_exchange(
    ...
    assert chaindrift.handle_stoploss_on_exchange(trade) is False
    assert stoploss.call_count == 1
    ...
    assert chaindrift.handle_stoploss_on_exchange(trade) is True
    ...
    trade.is_open = True
    chaindrift.handle_stoploss_on_exchange(trade)
...
def test_handle_stoploss_on_exchange_emergency(
    ...
    chaindrift.config["trailing_stop"] = True
...
def test_handle_stoploss_on_exchange_partial_cancel_here(
    ...
    # Chaindrift cancels the stop - but cancel returns a partial filled order.
    stoploss_order_hit = MagicMock(
        return_value={
            "id": "101",
            "status": "open",
...
def test_create_stoploss_order_invalid_order(
    ...
    caplog.clear()
    rpc_mock.reset_mock()
    chaindrift.create_stoploss_order(trade, 200)
    assert trade.has_open_sl_orders is False
...
def test_handle_stoploss_on_exchange_trailing(
    ...
    # setting stoploss
    chaindrift.strategy.stoploss = 0.05 if is_short else -0.05

    # setting stoploss_on_exchange_interval to 60 seconds
    chaindrift.strategy.order_types["stoploss_on_exchange_interval"] = 60
    ...
    stoploss_order_mock.assert_called_once_with(
        amount=30,
        pair="ETH/USDT",
        order_types=chaindrift.strategy.order_types,
...
def test_handle_stoploss_on_exchange_trailing_error(
    ...
    chaindrift.handle_trailing_stoploss_on_exchange(trade, stoploss_order_hanging)
...
def test_stoploss_on_exchange_price_rounding(
    ...
    chaindrift.handle_trailing_stoploss_on_exchange(open_trade_usdt, {})
...
def test_handle_stoploss_on_exchange_custom_stop(
    ...
    # setting stoploss
    chaindrift.strategy.custom_stoploss = lambda *args, **kwargs: -0.04
    ...
    chaindrift.strategy.order_types["stoploss_on_exchange_interval"] = 0
    cancel_order_mock.assert_not_called()
    stoploss_order_mock.assert_not_called()
...
def test_tsl_on_exchange_compatible_with_edge(mocker, edge_conf, fee, limit_order) -> None:
    ...
    # setting stoploss
    chaindrift.strategy.stoploss = -0.02
    ...
    chaindrift.active_pair_whitelist = chaindrift.edge.adjust(chaindrift.active_pair_whitelist)
...
def test_execute_trade_exit_sloe_cancel_exception(
    ...
    chaindrift = get_patched_chaindriftbot(mocker, default_conf_usdt)
    mocker.patch(f"{EXMS}.cancel_stoploss_order", side_effect=InvalidOrderException())
    mocker.patch("chaindrift.wallets.Wallets.get_free", MagicMock(return_value=300))
    create_order_mock = MagicMock(
        side_effect=[
            {"id": "12345554"},
            {"id": "12345555"},
...
def test_may_execute_trade_exit_after_stoploss_on_exchange_hit(
    ...
    chaindrift = ChaindriftBot(default_conf_usdt)
    chaindrift.strategy.order_types["stoploss_on_exchange"] = True
    patch_get_signal(chaindrift, enter_long=not is_short, enter_short=is_short)
