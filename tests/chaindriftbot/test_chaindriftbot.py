...
from chaindrift.constants import CANCEL_REASON, UNLIMITED_STAKE_AMOUNT
from chaindrift.enums import (
    CandleType,
    ExitCheckTuple,
    ExitType,
    RPCMessageType,
...
from chaindrift.exceptions import (
    DependencyException,
    ExchangeError,
...
from chaindrift.plugins.protections.iprotection import ProtectionReturn
from chaindrift.util.datetime_helpers import dt_now, dt_utc
from chaindrift.worker import Worker
from tests.conftest import (
    EXMS,
    create_mock_trades,
    create_mock_trades_usdt,
    get_patched_chaindriftbot,
    get_patched_worker,
    log_has,
    log_has_re,
    patch_edge,
...
def patch_RPCManager(mocker) -> MagicMock:
    ...
    """
    mocker.patch("chaindrift.rpc.telegram.Telegram", MagicMock())
    rpc_mock = mocker.patch("chaindrift.chaindriftbot.RPCManager.send_msg", MagicMock())
    return rpc_mock
...
def test_chaindriftbot_state(mocker, default_conf_usdt, markets) -> None:
    mocker.patch(f"{EXMS}.markets", PropertyMock(return_value=markets))
    chaindrift = get_patched_chaindriftbot(mocker, default_conf_usdt)
    assert chaindrift.state is State.RUNNING

    default_conf_usdt.pop("initial_state")
    chaindrift = ChaindriftBot(default_conf_usdt)
    assert chaindrift.state is State.STOPPED


def test_process_stopped(mocker, default_conf_usdt) -> None:
    chaindrift = get_patched_chaindriftbot(mocker, default_conf_usdt)
    coo_mock = mocker.patch("chaindrift.chaindriftbot.ChaindriftBot.cancel_all_open_orders")
    chaindrift.process_stopped()
...
def test_process_calls_sendmsg(mocker, default_conf_usdt) -> None:
    chaindrift = get_patched_chaindriftbot(mocker, default_conf_usdt)
    chaindrift.process()
    assert chaindrift.rpc.process_msg_queue.call_count == 1


def test_bot_cleanup(mocker, default_conf_usdt, caplog) -> None:
    mock_cleanup = mocker.patch("chaindrift.chaindriftbot.Trade.commit")
    coo_mock = mocker.patch("chaindrift.chaindriftbot.ChaindriftBot.cancel_all_open_orders")
    chaindrift = get_patched_chaindriftbot(mocker, default_conf_usdt)
    chaindrift.cleanup()
    ...
    chaindrift.config["cancel_open_orders_on_exit"] = True
    chaindrift.cleanup()
    assert coo_mock.call_count == 1


def test_bot_cleanup_db_errors(mocker, default_conf_usdt, caplog) -> None:
    mocker.patch("chaindrift.chaindriftbot.Trade.commit", side_effect=OperationalException())
    mocker.patch(
        "chaindrift.chaindriftbot.ChaindriftBot.check_for_open_trades",
        side_effect=OperationalException(),
    )
    chaindrift = get_patched_chaindriftbot(mocker, default_conf_usdt)
    chaindrift.emc = MagicMock()
    chaindrift.emc.shutdown = MagicMock()
    chaindrift.cleanup()
    assert chaindrift.emc.shutdown.call_count == 1
...
def test_order_dict(default_conf_usdt, mocker, runmode, caplog) -> None:
    ...
    chaindrift = ChaindriftBot(conf)
    if runmode == RunMode.LIVE:
        assert not log_has_re(r".*stoploss_on_exchange .* dry-run", caplog)
    assert chaindrift.strategy.order_types["stoploss_on_exchange"]
    ...
    chaindrift = ChaindriftBot(conf)
    assert not chaindrift.strategy.order_types["stoploss_on_exchange"]
    assert not log_has_re(r".*stoploss_on_exchange .* dry-run", caplog)
...
def test_get_trade_stake_amount(default_conf_usdt, mocker) -> None:
    ...
    result = chaindrift.wallets.get_trade_stake_amount("ETH/USDT", 1)
...
def test_load_strategy_no_keys(default_conf_usdt, mocker, runmode, caplog) -> None:
    ...
    conf["runmode"] = runmode
    erm = mocker.patch("chaindrift.chaindriftbot.ExchangeResolver.load_exchange")

    chaindrift = ChaindriftBot(conf)
    strategy_config = chaindrift.strategy.config
...
def test_check_available_stake_amount(
    ...
    for i in range(0, max_open):
        if expected[i] is not None:
            ...
            chaindrift.execute_entry("ETH/USDT", result)
        else:
            with pytest.raises(DependencyException):
                chaindrift.wallets.get_trade_stake_amount("ETH/USDT", 1)
...
def test_edge_called_in_process(mocker, edge_conf) -> None:
    ...
    patch_exchange(mocker)
    chaindrift = ChaindriftBot(edge_conf)
    patch_get_signal(chaindrift)
    chaindrift.process()
    assert chaindrift.active_pair_whitelist == ["NEO/BTC", "LTC/BTC"]
...
def test_edge_overrides_stake_amount(mocker, edge_conf) -> None:
    ...
    assert (
        chaindrift.wallets.get_trade_stake_amount("NEO/BTC", 1, chaindrift.edge)
        == (999.9 * 0.5 * 0.01) / 0.20
    )
    assert (
        chaindrift.wallets.get_trade_stake_amount("LTC/BTC", 1, chaindrift.edge)
        == (999.9 * 0.5 * 0.01) / 0.21
    )
...
def test_edge_overrides_stoploss(
    ...
    chaindrift = ChaindriftBot(edge_conf)
    chaindrift.active_pair_whitelist = ["NEO/BTC"]
    patch_get_signal(chaindrift)
    chaindrift.strategy.min_roi_reached = MagicMock(return_value=False)
    chaindrift.enter_positions()
    ...
    assert chaindrift.handle_trade(trade) is not ignore_strat_sl
...
def test_create_trade(
    ...
    chaindrift = ChaindriftBot(default_conf_usdt)
    patch_get_signal(chaindrift, enter_short=is_short, enter_long=not is_short)
    chaindrift.create_trade("ETH/USDT")
...
def test_create_trade_minimal_amount(
    ...
    chaindrift = ChaindriftBot(default_conf_usdt)
    chaindrift.config["stake_amount"] = stake_amount
    patch_get_signal(chaindrift, enter_short=is_short, enter_long=not is_short)

    if create:
        assert chaindrift.create_trade("ETH/USDT")
        if amount_enough:
        ...
        assert not chaindrift.create_trade("ETH/USDT")
        if not max_open_trades:
            assert (
                chaindrift.wallets.get_trade_stake_amount(
                    "ETH/USDT", default_conf_usdt["max_open_trades"], chaindrift.edge
                )
                == 0
            )
...
def test_enter_positions_no_pairs_left(
    ...
    mocker.patch("chaindrift.configuration.config_validation._validate_whitelist")
    default_conf_usdt["exchange"]["pair_whitelist"] = whitelist
    chaindrift = ChaindriftBot(default_conf_usdt)
    patch_get_signal(chaindrift)

    n = chaindrift.enter_positions()
    assert n == positions
    if positions:
...
def test_handle_protections(mocker, default_conf_usdt, fee, is_short):
    ...
    chaindrift = get_patched_chaindriftbot(mocker, default_conf_usdt)
    chaindrift.protections._protection_handlers[1].global_stop = MagicMock(
        return_value=ProtectionReturn(True, dt_now() + timedelta(hours=1), "asdf")
    )
    create_mock_trades(fee, is_short)
    chaindrift.handle_protections("ETC/BTC", "*")
    send_msg_mock = chaindrift.rpc.send_msg
...
def test_create_trade_no_signal(default_conf_usdt, fee, mocker) -> None:
    ...
    chaindrift = ChaindriftBot(default_conf_usdt)
    patch_get_signal(chaindrift, enter_long=False, exit_long=False)
...
def test_create_trades_preopen(
    ...
    chaindrift.execute_entry("ETH/USDT", default_conf_usdt["stake_amount"])
    chaindrift.execute_entry("NEO/BTC", default_conf_usdt["stake_amount"])
    ...
    assert chaindrift.create_trade("ETH/USDT")
    assert chaindrift.create_trade("NEO/BTC")
...
def test_process_trade_creation(
    ...
    mocker.patch("chaindrift.chaindriftbot.ChaindriftBot._check_and_execute_exit")
...
def test_process_exchange_failures(default_conf_usdt, ticker_usdt, mocker) -> None:
    ...
    patch_get_signal(worker.chaindrift)
...
def test_process_operational_exception(default_conf_usdt, ticker_usdt, mocker) -> None:
    ...
    assert worker.chaindrift.state == State.RUNNING

    worker._process_running()
    assert worker.chaindrift.state == State.STOPPED
...
def test_process_trade_no_whitelist_pair(
    ...
    assert pair not in chaindrift.active_pair_whitelist
    chaindrift.process()
    assert pair in chaindrift.active_pair_whitelist
    # Make sure each pair is only in the list once
    assert len(chaindrift.active_pair_whitelist) == len(set(chaindrift.active_pair_whitelist))
...
def test_process_informative_pairs_added(default_conf_usdt, ticker_usdt, mocker) -> None:
    ...
    mocker.patch.multiple(
        "chaindrift.strategy.interface.IStrategy",
    ...
    chaindrift = ChaindriftBot(default_conf_usdt)
    chaindrift.strategy.informative_pairs = inf_pairs
    # patch_get_signal(chaindrift)
...
def test_execute_entry(
    ...
    mocker.patch("chaindrift.exchange.gate.Gate.validate_ordertypes")
    patch_RPCManager(mocker)
    patch_exchange(mocker, exchange=exchange_name)
    chaindrift = ChaindriftBot(default_conf_usdt)
    chaindrift.strategy.confirm_trade_entry = MagicMock(return_value=False)
    chaindrift.strategy.leverage = MagicMock(return_value=leverage)
    stake_amount = 2
    bid = 0.11
    ...
    mocker.patch.multiple(
        "chaindrift.exchange.okx.Okx",
        get_max_pair_stake_amount=MagicMock(return_value=500000),
    )
    pair = "ETH/USDT"

    assert not chaindrift.execute_entry(pair, stake_amount, is_short=is_short)
    assert enter_rate_mock.call_count == 1
    assert enter_mm.call_count == 0
    assert chaindrift.strategy.confirm_trade_entry.call_count == 1
    enter_rate_mock.reset_mock()

    open_order["id"] = "22"
    chaindrift.strategy.confirm_trade_entry = MagicMock(return_value=True)
    assert chaindrift.execute_entry(pair, stake_amount)
    ...
    open_order["id"] = "33"
    fix_price = 0.06
    assert chaindrift.execute_entry(pair, stake_amount, fix_price, is_short=is_short)
    ...
    assert chaindrift.execute_entry(pair, stake_amount, is_short=is_short)
    ...
    chaindrift.strategy.custom_stake_amount = lambda **kwargs: 150.0
    assert chaindrift.execute_entry(pair, stake_amount, is_short=is_short)
    ...
    # Exception case
    order["id"] = "557"
    chaindrift.strategy.custom_stake_amount = lambda **kwargs: 20 / 0
    assert chaindrift.execute_entry(pair, stake_amount, is_short=is_short)
    ...
    order["id"] = "66"
    mocker.patch(f"{EXMS}.create_order", MagicMock(return_value=order))
    assert not chaindrift.execute_entry(pair, stake_amount)
    assert chaindrift.strategy.leverage.call_count == 0 if trading_mode == "spot" else 2
    ...
    with pytest.raises(PricingError, match="Could not determine entry price."):
        chaindrift.execute_entry(pair, stake_amount, is_short=is_short)

    # In case of custom entry price
    mocker.patch(f"{EXMS}.get_rate", return_value=0.50)
    order["status"] = "open"
    order["id"] = "5566"
    chaindrift.strategy.custom_entry_price = lambda **kwargs: 0.508
    assert chaindrift.execute_entry(pair, stake_amount, is_short=is_short)
    ...
    order["status"] = "open"
    order["id"] = "5567"
    chaindrift.strategy.custom_entry_price = lambda **kwargs: None
    ...
    order["status"] = "open"
    order["id"] = "5568"
    chaindrift.strategy.custom_entry_price = lambda **kwargs: "string price"
    assert chaindrift.execute_entry(pair, stake_amount, is_short=is_short)
    ...
    chaindrift.exchange.get_max_pair_stake_amount = MagicMock(return_value=500)

    assert chaindrift.execute_entry(pair, 2000, is_short=is_short)
    ...
    chaindrift.strategy.leverage.reset_mock()
    assert chaindrift.execute_entry(pair, 200, leverage_=3)
    assert chaindrift.strategy.leverage.call_count == 0
...
def test_execute_entry_confirm_error(mocker, default_conf_usdt, fee, limit_order, is_short) -> None:
    ...
    chaindrift.strategy.confirm_trade_entry = MagicMock(side_effect=ValueError)
    assert chaindrift.execute_entry(pair, stake_amount)

    limit_order[entry_side(is_short)]["id"] = "222"
    chaindrift.strategy.confirm_trade_entry = MagicMock(side_effect=Exception)
    assert chaindrift.execute_entry(pair, stake_amount)
...
def test_execute_entry_fully_canceled_on_create(
    ...
    mock_hce = mocker.spy(chaindrift, "handle_cancel_enter")
...
def test_execute_entry_min_leverage(mocker, default_conf_usdt, fee, limit_order, is_short) -> None:
    ...
    stake_amount = 2
    pair = "SOL/BUSD:BUSD"
    chaindrift.strategy.leverage = MagicMock(return_value=5.0)
...
def test_enter_positions(
    ...
    mock_ct = mocker.patch(
        "chaindrift.chaindriftbot.ChaindriftBot.create_trade",
...
def test_exit_positions(mocker, default_conf_usdt, limit_order, is_short, caplog) -> None:
    ...
    mocker.patch("chaindrift.chaindriftbot.ChaindriftBot.handle_trade", MagicMock(return_value=True))
    ...
    Trade.commit()
    trades = [trade]
    chaindrift.wallets.update()
    n = chaindrift.exit_positions(trades)
    assert n == 0
    # Test amount not modified by fee-logic
    assert not log_has_re(r"Applying fee to amount for Trade .*", caplog)

    gra = mocker.patch("chaindrift.chaindriftbot.ChaindriftBot.get_real_amount", return_value=0.0)
...
def test_exit_positions_exception(mocker, default_conf_usdt, limit_order, caplog, is_short) -> None:
    ...
    mocker.patch(
        "chaindrift.chaindriftbot.ChaindriftBot.handle_trade", side_effect=DependencyException()
    )
    caplog.clear()
    n = chaindrift.exit_positions(trades)
    assert n == 0
...
def test_update_trade_state(mocker, default_conf_usdt, limit_order, is_short, caplog) -> None:
    ...
    mocker.patch("chaindrift.chaindriftbot.ChaindriftBot.handle_trade", MagicMock(return_value=True))
    mocker.patch("chaindrift.chaindriftbot.ChaindriftBot._notify_enter")
    mocker.patch(f"{EXMS}.fetch_order", return_value=order)
    mocker.patch(f"{EXMS}.get_trades_for_order", return_value=[])
    mocker.patch("chaindrift.chaindriftbot.ChaindriftBot.get_real_amount", return_value=0.0)
    order_id = order["id"]
    ...
    chaindrift.strategy.order_filled = MagicMock(return_value=None)
    assert not chaindrift.update_trade_state(trade, None)
    assert log_has_re(r"Orderid for trade .* is empty.", caplog)
    caplog.clear()
    # Add datetime explicitly since sqlalchemy defaults apply only once written to database
    chaindrift.update_trade_state(trade, order_id)
    # Test amount not modified by fee-logic
    assert not log_has_re(r"Applying fee to .*", caplog)
    caplog.clear()
    assert not trade.has_open_orders
    assert trade.amount == order["amount"]
    assert chaindrift.strategy.order_filled.call_count == 1

    mocker.patch("chaindrift.chaindriftbot.ChaindriftBot.get_real_amount", return_value=0.01)
    assert trade.amount == 30.0
    # test amount modified by fee-logic
    chaindrift.update_trade_state(trade, order_id)
    ...
    chaindrift.strategy.order_filled = MagicMock(return_value=None)
    mocker.patch("chaindrift.chaindriftbot.ChaindriftBot.get_real_amount", side_effect=ValueError)
    mocker.patch(f"{EXMS}.fetch_order", return_value=limit_buy_order_usdt_new)
    res = chaindrift.update_trade_state(trade, order_id)
    # Cancelled empty
    assert res is True
    assert chaindrift.strategy.order_filled.call_count == 0
...
def test_update_trade_state_withorderdict(
    ...
    chaindrift.update_trade_state(trade, order_id, order)
...
def test_update_trade_state_exception(
    ...
    mocker.patch(
        "chaindrift.chaindriftbot.ChaindriftBot.get_real_amount", side_effect=DependencyException()
    )
    chaindrift.update_trade_state(trade, open_order_id)
...
def test_update_trade_state_orderexception(mocker, default_conf_usdt, caplog) -> None:
    ...
    # Test raise of OperationalException exception
    grm_mock = mocker.patch("chaindrift.chaindriftbot.ChaindriftBot.get_real_amount", MagicMock())
    chaindrift.update_trade_state(trade, open_order_id)
...
def test_update_trade_state_sell(
    ...
    mocker.patch("chaindrift.wallets.Wallets.update", wallet_mock)
    ...
    chaindrift.update_trade_state(trade, trade.open_orders_ids[-1], l_order)
...
def test_handle_trade(
    ...
    assert chaindrift.handle_trade(trade) is True
...
def test_handle_overlapping_signals(
    ...
    chaindrift = ChaindriftBot(default_conf_usdt)
    if is_short:
        patch_get_signal(chaindrift, enter_long=False, enter_short=True, exit_short=True)
    else:
        patch_get_signal(chaindrift, enter_long=True, exit_long=True)
    chaindrift.strategy.min_roi_reached = MagicMock(return_value=False)
    ...
    patch_get_signal(chaindrift, enter_long=False)
    assert chaindrift.handle_trade(trades[0]) is False
    trades = Trade.session.scalars(select(Trade)).all()
    for trade in trades:
        trade.is_short = is_short
    nb_trades = len(trades)
    ...
    if is_short:
        patch_get_signal(chaindrift, enter_long=False, exit_short=True)
    else:
        patch_get_signal(chaindrift, enter_long=False, exit_long=True)
    trades = Trade.session.scalars(select(Trade)).all()
    for trade in trades:
        trade.is_short = is_short
    assert chaindrift.handle_trade(trades[0]) is True
...
def test_handle_trade_roi(
    ...
    chaindrift = get_patched_chaindriftbot(mocker, default_conf_usdt)
    patch_get_signal(chaindrift, enter_short=is_short, enter_long=not is_short)
    chaindrift.strategy.min_roi_reached = MagicMock(return_value=True)
    ...
    caplog.clear()
    patch_get_signal(chaindrift)
    assert chaindrift.handle_trade(trade)
...
def test_handle_trade_use_exit_signal(
    ...
    patch_get_signal(chaindrift, enter_long=False, exit_long=False)
    assert not chaindrift.handle_trade(trade)
...
def test_close_trade(
    ...
    with pytest.raises(DependencyException, match=r".*closed trade.*"):
        chaindrift.handle_trade(trade)


def test_bot_loop_start_called_once(mocker, default_conf_usdt, caplog):
    ftbot = get_patched_chaindriftbot(mocker, default_conf_usdt)
    mocker.patch("chaindrift.chaindriftbot.ChaindriftBot.create_trade")
...
def test_manage_open_orders_entry_usercustom(
    ...
    chaindrift.manage_open_orders()
    assert cancel_order_mock.call_count == 0

    # Return false - trade remains open
    chaindrift.strategy.check_entry_timeout = MagicMock(return_value=False)
    chaindrift.manage_open_orders()
    assert cancel_order_mock.call_count == 0
    ...
    nb_trades = len(trades)
    assert nb_trades == 1
    assert chaindrift.strategy.check_entry_timeout.call_count == 1
    chaindrift.strategy.check_entry_timeout = MagicMock(side_effect=KeyError)
    ...
    assert nb_trades == 1
    assert chaindrift.strategy.check_entry_timeout.call_count == 1
    chaindrift.strategy.check_entry_timeout = MagicMock(return_value=True)
...
def test_manage_open_orders_entry(
    ...
    chaindrift.strategy.check_entry_timeout = MagicMock(return_value=False)
    chaindrift.strategy.adjust_entry_price = MagicMock(return_value=1234)
    ...
    assert chaindrift.strategy.check_entry_timeout.call_count == 0
    # Entry adjustment is never called
    assert chaindrift.strategy.adjust_entry_price.call_count == 0
...
def test_adjust_entry_cancel(
    ...
    chaindrift.strategy.ft_check_timed_out = MagicMock(return_value=False)

    # check that order is cancelled
    chaindrift.strategy.adjust_entry_price = MagicMock(return_value=None)
    chaindrift.manage_open_orders()
    ...
    assert chaindrift.strategy.adjust_entry_price.call_count == 1
...
def test_adjust_entry_replace_fail(
    ...
    mocker.patch("chaindrift.chaindriftbot.sleep")
    ...
    chaindrift.strategy.adjust_entry_price = MagicMock(return_value=12234)
    chaindrift.manage_open_orders()
    trades = Trade.session.scalars(select(Trade).where(Order.ft_trade_id == Trade.id)).all()
...
def test_adjust_entry_replace_fail_create_order(
    ...
    mocker.patch("chaindrift.chaindriftbot.sleep")
    mocker.patch(
        "chaindrift.chaindriftbot.ChaindriftBot.execute_entry", side_effect=DependencyException()
    )
...
def test_adjust_entry_maintain_replace(
    ...
    chaindrift.strategy.adjust_entry_price = MagicMock(return_value=old_order["price"])
    chaindrift.manage_open_orders()
    trades = Trade.session.scalars(
    ...
    chaindrift.get_valid_enter_price_and_stake = MagicMock(return_value={100, 10, 1})
    chaindrift.strategy.adjust_entry_price = MagicMock(return_value=1234)
...
def test_manage_open_orders_exit_usercustom(
    ...
    et_mock = mocker.patch("chaindrift.chaindriftbot.ChaindriftBot.execute_trade_exit")
    mocker.patch.multiple(
        EXMS,
        fetch_ticker=ticker_usdt,
    ...
    chaindrift.strategy.check_exit_timeout = MagicMock(return_value=False)
    chaindrift.strategy.check_entry_timeout = MagicMock(return_value=False)
    # Return false - No impact
    chaindrift.manage_open_orders()
    assert cancel_order_mock.call_count == 0
    assert rpc_mock.call_count == 1
    assert chaindrift.strategy.check_exit_timeout.call_count == 1
    assert chaindrift.strategy.check_entry_timeout.call_count == 0

    chaindrift.strategy.check_exit_timeout = MagicMock(side_effect=KeyError)
    chaindrift.strategy.check_entry_timeout = MagicMock(side_effect=KeyError)
    ...
    # Return True - sells!
    chaindrift.strategy.check_exit_timeout = MagicMock(return_value=True)
    chaindrift.strategy.check_entry_timeout = MagicMock(return_value=True)
    chaindrift.manage_open_orders()
    assert cancel_order_mock.call_count == 1
    assert rpc_mock.call_count == 2
    ...
    mocker.patch("chaindrift.persistence.Trade.get_canceled_exit_order_count", return_value=1)
    mocker.patch(
        "chaindrift.chaindriftbot.ChaindriftBot.execute_trade_exit", side_effect=DependencyException
    )
    chaindrift.manage_open_orders()
    ...
    with patch("chaindrift.chaindriftbot.ChaindriftBot.handle_cancel_exit", return_value=False):
        chaindrift.manage_open_orders()
...
def test_manage_open_orders_exit(
    ...
    assert chaindrift.strategy.check_exit_timeout.call_count == 0
    assert chaindrift.strategy.check_entry_timeout.call_count == 0
...
def test_manage_open_orders_partial_fee(
    ...
    mocker.patch("chaindrift.wallets.Wallets.get_free", MagicMock(return_value=0))
    patch_exchange(mocker)
    mocker.patch.multiple(
        EXMS,
        fetch_ticker=ticker_usdt,
...
def test_manage_open_orders_partial_except(
    ...
    mocker.patch(
        "chaindrift.chaindriftbot.ChaindriftBot.get_real_amount",
...
def test_manage_open_orders_exception(
    ...
    mocker.patch.multiple(
        "chaindrift.chaindriftbot.ChaindriftBot",
...
def test_handle_cancel_enter(mocker, caplog, default_conf_usdt, limit_order, is_short, fee) -> None:
    ...
    chaindrift = ChaindriftBot(default_conf_usdt)
    chaindrift._notify_enter_cancel = MagicMock()
    ...
    assert chaindrift.handle_cancel_enter(trade, l_order, trade.open_orders[0], reason)
    assert cancel_order_mock.call_count == 1

    cancel_order_mock.reset_mock()
    caplog.clear()
    l_order["filled"] = 0.01
    assert not chaindrift.handle_cancel_enter(trade, l_order, trade.open_orders[0], reason)
    assert cancel_order_mock.call_count == 0
    ...
    assert not chaindrift.handle_cancel_enter(
...
def test_handle_cancel_enter_exchanges(
    ...
    notify_mock = mocker.patch("chaindrift.chaindriftbot.ChaindriftBot._notify_enter_cancel")
    chaindrift = ChaindriftBot(default_conf_usdt)
    ...
    Trade.session.add(trade)
    Trade.commit()
    assert chaindrift.handle_cancel_enter(
...
def test_handle_cancel_exit_limit(
    ...
    mocker.patch("chaindrift.chaindriftbot.ChaindriftBot.handle_order_fee")
    ...
    assert chaindrift.handle_cancel_exit(trade, order, order_obj, reason)
    assert cancel_order_mock.call_count == 1
    ...
    assert not chaindrift.handle_cancel_exit(trade, order, order_obj, reason)
...
def test_handle_cancel_exit_cancel_exception(mocker, default_conf_usdt) -> None:
    ...
    # assert not chaindrift.handle_cancel_exit(trade, order, reason)
...
def test_execute_trade_exit_up(
    ...
    chaindrift = ChaindriftBot(default_conf_usdt)
    patch_get_signal(chaindrift, enter_short=is_short, enter_long=not is_short)
    chaindrift.strategy.confirm_trade_exit = MagicMock(return_value=False)
    ...
    assert trade
    assert chaindrift.strategy.confirm_trade_exit.call_count == 0
    ...
    assert chaindrift.strategy.confirm_trade_exit.call_count == 1
    assert id(chaindrift.strategy.confirm_trade_exit.call_args_list[0][1]["trade"]) != id(trade)
    assert chaindrift.strategy.confirm_trade_exit.call_args_list[0][1]["trade"].id == trade.id

    # Repatch with true
    chaindrift.strategy.confirm_trade_exit = MagicMock(return_value=True)
    chaindrift.execute_trade_exit(
        trade=trade,
        limit=(ticker_usdt_sell_down()["ask"] if is_short else ticker_usdt_sell_up()["bid"]),
        exit_check=ExitCheckTuple(exit_type=ExitType.ROI),
...
def test_execute_trade_exit_custom_exit_price(
    ...
    chaindrift = ChaindriftBot(config)
    patch_get_signal(chaindrift, enter_short=is_short, enter_long=not is_short)
    chaindrift.strategy.confirm_trade_exit = MagicMock(return_value=False)
    ...
    chaindrift.strategy.custom_exit_price = lambda **kwargs: 2.25
    chaindrift.execute_trade_exit(
        trade=trade,
...
def test_execute_trade_exit_market_order(
    ...
    chaindrift.config["order_types"]["exit"] = "market"
...
def test_execute_trade_exit_insufficient_funds_error(
    ...
    chaindrift = get_patched_chaindriftbot(mocker, default_conf_usdt)
    mock_insuf = mocker.patch("chaindrift.chaindriftbot.ChaindriftBot.handle_insufficient_funds")
    mocker.patch.multiple(
        EXMS,
        fetch_ticker=ticker_usdt,
        get_fee=fee,
    ...
    assert not chaindrift.execute_trade_exit(
        trade=trade,
        limit=ticker_usdt_sell_up()["ask" if is_short else "bid"],
...
def test_exit_profit_only(
    ...
    chaindrift = ChaindriftBot(default_conf_usdt)
    patch_get_signal(chaindrift, enter_short=is_short, enter_long=not is_short)
    chaindrift.strategy.custom_exit = MagicMock(return_value=None)
    if exit_type == ExitType.EXIT_SIGNAL.value:
        chaindrift.strategy.min_roi_reached = MagicMock(return_value=False)
    else:
        chaindrift.strategy.ft_stoploss_reached = MagicMock(
    ...
    chaindrift.wallets.update()
    if profit_only:
        assert chaindrift.handle_trade(trade) is False
        # Custom-exit is called
        assert chaindrift.strategy.custom_exit.call_count == 1

    patch_get_signal(chaindrift, enter_long=False, exit_short=is_short, exit_long=not is_short)
    assert chaindrift.handle_trade(trade) is handle_first

    if handle_second:
        chaindrift.strategy.exit_profit_offset = 0.0
        assert chaindrift.handle_trade(trade) is True
...
def test_sell_not_enough_balance(
    ...
    trade.update_trade(oobj)
    patch_get_signal(chaindrift, enter_long=False, exit_long=True)
    mocker.patch("chaindrift.wallets.Wallets.get_free", MagicMock(return_value=trade.amount * 0.985))
...
def test__safe_exit_amount(default_conf_usdt, fee, caplog, mocker, amount_wallet, has_err):
    ...
    mocker.patch("chaindrift.wallets.Wallets.get_free", MagicMock(return_value=amount_wallet))
    wallet_update = mocker.patch("chaindrift.wallets.Wallets.update")
    trade = Trade(
        pair="LTC/ETH",
        amount=amount,
        exchange="binance",
    ...
    if has_err:
        with pytest.raises(DependencyException, match=r"Not enough amount to exit trade."):
            assert chaindrift._safe_exit_amount(trade, trade.pair, trade.amount)
    else:
        wallet_update.reset_mock()
        assert trade.amount != amount_wallet
        assert chaindrift._safe_exit_amount(trade, trade.pair, trade.amount) == amount_wallet
        assert log_has_re(r".*Falling back to wallet-amount.", caplog)
        assert trade.amount == amount_wallet
        assert wallet_update.call_count == 1
        caplog.clear()
        wallet_update.reset_mock()
        assert chaindrift._safe_exit_amount(trade, trade.pair, amount_wallet) == amount_wallet
...
def test_locked_pairs(
    ...
    assert not chaindrift.strategy.is_pair_locked(trade.pair, side="*")
    # Both sides are locked
    assert chaindrift.strategy.is_pair_locked(trade.pair, side="long") != is_short
    assert chaindrift.strategy.is_pair_locked(trade.pair, side="short") == is_short
...
def test_ignore_roi_if_entry_signal(
    ...
    if is_short:
        patch_get_signal(chaindrift, enter_long=False, exit_short=False, exit_tag="something")
    else:
        patch_get_signal(chaindrift, enter_long=False, exit_long=False, exit_tag="something")
    assert chaindrift.handle_trade(trade) is True
...
def test_disable_ignore_roi_if_entry_signal(
    ...
    patch_get_signal(chaindrift, enter_long=not is_short, enter_short=is_short, exit_short=is_short)
    assert chaindrift.handle_trade(trade) is True
...
def test_get_real_amount_quote(
    ...
    assert chaindrift.get_real_amount(trade, buy_order_fee, order_obj) == (amount * 0.001)
    assert log_has(
...
def test_get_real_amount_quote_dust(
    ...
    mocker.patch(f"{EXMS}.get_trades_for_order", return_value=trades_for_order)
    walletmock = mocker.patch("chaindrift.wallets.Wallets.update")
    mocker.patch("chaindrift.wallets.Wallets.get_free", return_value=8.1122)
    amount = sum(x["amount"] for x in trades_for_order)
    trade = Trade(
        pair="LTC/ETH",
        amount=amount,
    ...
    assert chaindrift.get_real_amount(trade, buy_order_fee, order_obj) is None
...
def test_get_real_amount(
    ...
    caplog.clear()
    order_obj = Order.parse_from_ccxt_object(buy_order_fee, "LTC/ETH", "buy")
    res = chaindrift.get_real_amount(trade, buy_order, order_obj)
...
def test_get_real_amount_multi(
    ...
    order_obj = Order.parse_from_ccxt_object(buy_order_fee, "LTC/ETH", "buy")
    assert chaindrift.get_real_amount(trade, buy_order_fee, order_obj) == expected_amount
    assert log_has(
        (
            "Applying fee on amount for Trade(id=None, pair=LTC/ETH, amount=8.00000000, "
...
def test_get_real_amount_invalid_order(
    ...
    assert chaindrift.get_real_amount(trade, limit_buy_order_usdt, order_obj) is None
...
def test_get_real_amount_fees_order(
    ...
    assert chaindrift.get_real_amount(trade, market_buy_order_usdt_doublefee, order_obj) is None
...
def test_get_real_amount_wrong_amount(
    ...
    with pytest.raises(DependencyException, match=r"Half bought\? Amounts don't match"):
        chaindrift.get_real_amount(trade, limit_buy_order_usdt, order_obj)
...
def test_get_real_amount_wrong_amount_rounding(
    ...
    assert pytest.approx(chaindrift.get_real_amount(trade, limit_buy_order_usdt, order_obj)) == (
        amount * 0.001
    )
...
def test_get_real_amount_open_trade_usdt(default_conf_usdt, fee, mocker):
    ...
    assert chaindrift.get_real_amount(trade, order, order_obj) is None
...
def test_get_real_amount_in_point(default_conf_usdt, buy_order_fee, fee, mocker, caplog):
    ...
    order_obj = Order.parse_from_ccxt_object(buy_order_fee, "LTC/ETH", "buy")
    res = chaindrift.get_real_amount(trade, limit_buy_order_usdt, order_obj)
    assert res is None
    ...
    caplog.clear()
    chaindrift.config["exchange"]["unknown_fee_rate"] = 1
    res = chaindrift.get_real_amount(trade, limit_buy_order_usdt, order_obj)
    assert res is None
...
def test_apply_fee_conditional(
    ...
    walletmock = mocker.patch("chaindrift.wallets.Wallets.update")
    mocker.patch("chaindrift.wallets.Wallets.get_free", return_value=wallet)
    trade = Trade(
        pair="LTC/ETH",
        amount=amount,
        exchange="binance",
    ...
    walletmock.reset_mock()
    # Amount is kept as is
    assert chaindrift.apply_fee_conditional(trade, "LTC", amount, fee_abs, order) == amount_exp
    assert walletmock.call_count == 1
...
def test_apply_fee_conditional_multibuy(
    ...
    assert chaindrift.apply_fee_conditional(trade, "LTC", amount, fee_abs, order1) == amount_exp
    assert walletmock.call_count == 1
    if fee_abs != 0 and amount_exp is None:
        assert log_has_re(r"Fee amount.*Eating.*dust\.", caplog)
...
def test_order_book_entry_pricing1(
    ...
    if exception_thrown:
        with pytest.raises(PricingError):
            chaindrift.exchange.get_rate("ETH/USDT", side="entry", is_short=False, refresh=True)
        assert log_has_re(
...
def test_check_depth_of_market(default_conf_usdt, mocker, order_book_l2) -> None:
    ...
    assert chaindrift._check_depth_of_market("ETH/BTC", conf, side=SignalDirection.LONG) is False
...
def test_startup_state(default_conf_usdt, mocker):
    ...
    assert worker.chaindrift.state is State.RUNNING


def test_startup_trade_reinit(default_conf_usdt, edge_conf, mocker):
    mocker.patch(f"{EXMS}.exchange_has", MagicMock(return_value=True))
    reinit_mock = MagicMock()
    mocker.patch("chaindrift.persistence.Trade.stoploss_reinitialization", reinit_mock)
    ...
    ftbot = get_patched_chaindriftbot(mocker, edge_conf)
    ftbot.startup()
...
def test_sync_wallet_dry_run(
    ...
    bot = get_patched_chaindriftbot(mocker, default_conf_usdt)
...
def test_cancel_all_open_orders(
    ...
    buy_mock = mocker.patch("chaindrift.chaindriftbot.ChaindriftBot.handle_cancel_enter")
    sell_mock = mocker.patch("chaindrift.chaindriftbot.ChaindriftBot.handle_cancel_exit")

    chaindrift = get_patched_chaindriftbot(mocker, default_conf_usdt)
    create_mock_trades(fee, is_short=is_short)
    trades = Trade.session.scalars(select(Trade)).all()
    assert len(trades) == MOCK_TRADE_COUNT
    chaindrift.cancel_all_open_orders()
...
def test_check_for_open_trades(mocker, default_conf_usdt, fee, is_short):
    ...
    chaindrift.check_for_open_trades()
    assert chaindrift.rpc.send_msg.call_count == 0
    ...
    chaindrift.check_for_open_trades()
    assert chaindrift.rpc.send_msg.call_count == 1
    assert "Handle these trades manually" in chaindrift.rpc.send_msg.call_args[0][0]["status"]
...
def test_startup_update_open_orders(mocker, default_conf_usdt, fee, caplog, is_short):
    ...
    chaindrift.startup_update_open_orders()
    assert not log_has_re(r"Error updating Order .*", caplog)
    caplog.clear()

    chaindrift.config["dry_run"] = False
    chaindrift.startup_update_open_orders()
    ...
    hto_mock = mocker.patch("chaindrift.chaindriftbot.ChaindriftBot.handle_cancel_order")
...
def test_startup_backpopulate_precision(mocker, default_conf_usdt, fee, caplog):
    ...
    chaindrift.startup_backpopulate_precision()
    trades = Trade.get_trades().all()
    for trade in trades:
...
def test_update_trades_without_assigned_fees(mocker, default_conf_usdt, fee, is_short):
    ...
    chaindrift.update_trades_without_assigned_fees()
...
def test_reupdate_enter_order_fees(mocker, default_conf_usdt, fee, caplog, is_short):
    chaindrift = get_patched_chaindriftbot(mocker, default_conf_usdt)
    mock_uts = mocker.patch("chaindrift.chaindriftbot.ChaindriftBot.update_trade_state")

    mocker.patch(f"{EXMS}.fetch_order_or_stoploss_order", return_value={"status": "open"})
    create_mock_trades(fee, is_short)
    trades = Trade.get_trades().all()

    chaindrift.handle_insufficient_funds(trades[3])
    ...
    chaindrift.handle_insufficient_funds(trade)
    # assert log_has_re(r"Trying to reupdate buy fees for .*", caplog)
...
def test_handle_insufficient_funds(mocker, default_conf_usdt, fee, is_short, caplog):
    ...
    chaindrift.handle_insufficient_funds(trades[4])
...
def test_handle_onexchange_order(mocker, default_conf_usdt, limit_order, is_short, caplog):
    ...
    chaindrift = get_patched_chaindriftbot(mocker, default_conf_usdt)
    mock_uts = mocker.spy(chaindrift, "update_trade_state")
    ...
    Trade.session.add(trade)
    chaindrift.handle_onexchange_order(trade)
...
def test_handle_onexchange_order_changed_amount(
    ...
    chaindrift.wallets = MagicMock()
    chaindrift.wallets.get_owned = MagicMock(return_value=entry_order["amount"] * factor)
...
def test_handle_onexchange_order_fully_canceled_enter(
    ...
    trade.orders.append(Order.parse_from_ccxt_object(entry_order, "ADA/USDT", entry_side(is_short)))
    Trade.session.add(trade)
    assert chaindrift.handle_onexchange_order(trade) is True
...
def test_get_valid_price(mocker, default_conf_usdt) -> None:
    ...
    patch_exchange(mocker)
    chaindrift = ChaindriftBot(default_conf_usdt)
    chaindrift.config["custom_price_max_distance_ratio"] = 0.02
    ...
    valid_price_from_string = chaindrift.get_valid_price(custom_price_string, proposed_price)
    valid_price_from_badstring = chaindrift.get_valid_price(custom_price_badstring, proposed_price)
    valid_price_from_int = chaindrift.get_valid_price(custom_price_int, proposed_price)
    valid_price_from_float = chaindrift.get_valid_price(custom_price_float, proposed_price)

    valid_price_at_max_alwd = chaindrift.get_valid_price(custom_price_over_max_alwd, proposed_price)
    valid_price_at_min_alwd = chaindrift.get_valid_price(custom_price_under_min_alwd, proposed_price)
...
def test_update_funding_fees_schedule(
    ...
    patch_RPCManager(mocker)
    patch_exchange(mocker)
    mocker.patch("chaindrift.chaindriftbot.ChaindriftBot.update_funding_fees", return_value=True)
    default_conf["trading_mode"] = trading_mode
    default_conf["margin_mode"] = "isolated"
    chaindrift = get_patched_chaindriftbot(mocker, default_conf)

    time_machine.move_to(f"{t2} {tzoffset}", tick=False)
    # Check schedule jobs in debugging with chaindrift._schedule.jobs
    chaindrift._schedule.run_pending()

    assert chaindrift.update_funding_fees.call_count == calls
...
def test_update_funding_fees(
    ...
    chaindrift.execute_entry("ETH/USDT", 123, is_short=is_short)
    chaindrift.execute_entry("LTC/USDT", 2.0, is_short=is_short)
    chaindrift.execute_entry("XRP/USDT", 123, is_short=is_short)
...
def test_update_funding_fees_error(mocker, default_conf, caplog):
    ...
    default_conf["margin_mode"] = "isolated"
    chaindrift = get_patched_chaindriftbot(mocker, default_conf)
    chaindrift.update_funding_fees()
...
def test_position_adjust(mocker, default_conf_usdt, fee) -> None:
    ...
    assert chaindrift.execute_entry(pair, stake_amount, trade=trade)
    ...
    assert chaindrift.execute_trade_exit(
        trade=trade,
        limit=8,
...
def test_position_adjust2(mocker, default_conf_usdt, fee) -> None:
    ...
    assert chaindrift.execute_entry(pair, amount)
    # Should create an closed trade with an no open order id
    # Order is filled and trade is open
    orders = Order.session.scalars(select(Order)).all()
    assert orders
...
def test_position_adjust3(mocker, default_conf_usdt, fee, data) -> None:
    ...
    for idx, (order, result) in enumerate(data):
        ...
        if order[0] == "buy":
            assert chaindrift.execute_entry(pair, amount, trade=trade)
        else:
            assert chaindrift.execute_trade_exit(
                trade=trade,
                limit=price,
...
def test_process_open_trade_positions_exception(mocker, default_conf_usdt, fee, caplog) -> None:
    ...
    mocker.patch(
        "chaindrift.chaindriftbot.ChaindriftBot.check_and_call_adjust_trade_position",
        side_effect=DependencyException(),
    )

    create_mock_trades(fee)

    chaindrift.process_open_trade_positions()
...
def test_check_and_call_adjust_trade_position(mocker, default_conf_usdt, fee, caplog) -> None:
    ...
    create_mock_trades(fee)
    caplog.set_level(logging.DEBUG)
    chaindrift.strategy.adjust_trade_position = MagicMock(return_value=(10, "aaaa"))
    chaindrift.process_open_trade_positions()
    assert log_has_re(r"Max adjustment entries for .* has been reached\.", caplog)
    assert chaindrift.strategy.adjust_trade_position.call_count == 1

    caplog.clear()
    chaindrift.strategy.adjust_trade_position = MagicMock(return_value=(-0.0005, "partial_exit_c"))
    chaindrift.process_open_trade_positions()
