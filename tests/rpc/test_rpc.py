...
from chaindrift.enums import SignalDirection, State, TradingMode
from chaindrift.exceptions import ExchangeError, InvalidOrderException, TemporaryError
from chaindrift.persistence import Order, Trade
from chaindrift.persistence.key_value_store import set_startup_time
from chaindrift.rpc import RPC, RPCException
...
def test_rpc_trade_status(default_conf, ticker, fee, mocker) -> None:
    ...
    # Fill open order ...
    chaindriftbot.manage_open_orders()
    trades = Trade.get_open_trades()
    chaindriftbot.exit_positions(trades)
...
def test__rpc_timeunit_profit(
    ...
    chaindriftbot = get_patched_chaindriftbot(mocker, default_conf_usdt)
...
def test_rpc_delete_trade(mocker, default_conf, fee, markets, caplog, is_short):
    ...
    chaindriftbot = get_patched_chaindriftbot(mocker, default_conf)
    chaindriftbot.strategy.order_types["stoploss_on_exchange"] = True
    create_mock_trades(fee, is_short)
    rpc = RPC(chaindriftbot)
...
def test_rpc_balance_handle(default_conf_usdt, mocker, tickers):
    ...
    mocker.patch("chaindrift.rpc.rpc.CryptoToFiatConverter._find_price", return_value=1.2)
    mocker.patch("chaindrift.rpc.telegram.Telegram", MagicMock())
    mocker.patch.multiple(
        EXMS,
...
def test_rpc_force_exit(default_conf, ticker, fee, mocker) -> None:
    ...
    chaindriftbot.config["max_open_trades"] = 3
    chaindriftbot.enter_positions()
...
def test_rpc_force_entry(mocker, default_conf, ticker, fee, limit_buy_order_open) -> None:
    ...
    chaindriftbot.config["max_open_trades"] = 5
    ...
    chaindriftbot.strategy.position_adjustment_enable = True
    ...
    pair = "XRP/BTC"
    chaindriftbot = get_patched_chaindriftbot(mocker, default_conf)
    chaindriftbot.config["stake_amount"] = 0
    patch_get_signal(chaindriftbot)
    rpc = RPC(chaindriftbot)
    pair = "TKN/BTC"
...
def test_rpc_edge_enabled(mocker, edge_conf) -> None:
    ...
    chaindriftbot = get_patched_chaindriftbot(mocker, edge_conf)
