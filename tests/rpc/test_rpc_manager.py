...
from chaindrift.enums import RPCMessageType
from chaindrift.rpc import RPCManager
from chaindrift.rpc.api_server.webserver import ApiServer
from tests.conftest import get_patched_chaindriftbot, log_has


def test__init__(mocker, default_conf) -> None:
    default_conf["telegram"]["enabled"] = False

    rpc_manager = RPCManager(get_patched_chaindriftbot(mocker, default_conf))
...
def test_cleanup_telegram_disabled(mocker, default_conf, caplog) -> None:
    caplog.set_level(logging.DEBUG)
    telegram_mock = mocker.patch("chaindrift.rpc.telegram.Telegram.cleanup", MagicMock())
    default_conf["telegram"]["enabled"] = False

    chaindriftbot = get_patched_chaindriftbot(mocker, default_conf)
    rpc_manager = RPCManager(chaindriftbot)
    rpc_manager.cleanup()
...
def test_send_msg_telegram_disabled(mocker, default_conf, caplog) -> None:
    telegram_mock = mocker.patch("chaindrift.rpc.telegram.Telegram.send_msg", MagicMock())
    default_conf["telegram"]["enabled"] = False
...
def test_send_msg_telegram_error(mocker, default_conf, caplog) -> None:
    mocker.patch("chaindrift.rpc.telegram.Telegram._init", MagicMock())
    mocker.patch("chaindrift.rpc.telegram.Telegram.send_msg", side_effect=ValueError())
    default_conf["telegram"]["enabled"] = True
    chaindriftbot = get_patched_chaindriftbot(mocker, default_conf)
    rpc_manager = RPCManager(chaindriftbot)
    rpc_manager.send_msg({"type": RPCMessageType.STATUS, "status": "test"})
...
def test_process_msg_queue(mocker, default_conf, caplog) -> None:
    telegram_mock = mocker.patch("chaindrift.rpc.telegram.Telegram.send_msg")
    default_conf["telegram"]["enabled"] = True
...
def test_send_msg_webhook_CustomMessagetype(mocker, default_conf, caplog) -> None:
    ...
    default_conf["webhook"] = {"enabled": True, "url": "https://DEADBEEF.com"}
    mocker.patch(
        "chaindrift.rpc.webhook.Webhook.send_msg", MagicMock(side_effect=NotImplementedError)
    )
    rpc_manager = RPCManager(get_patched_chaindriftbot(mocker, default_conf))
...
def test_startupmessages_telegram_enabled(mocker, default_conf) -> None:
    ...
    chaindriftbot = get_patched_chaindriftbot(mocker, default_conf)
    rpc_manager = RPCManager(chaindriftbot)
    rpc_manager.startup_messages(default_conf, chaindriftbot.pairlists, chaindriftbot.protections)
...
def test_init_apiserver_disabled(mocker, default_conf, caplog) -> None:
    caplog.set_level(logging.DEBUG)
    run_mock = MagicMock()
    mocker.patch("chaindrift.rpc.api_server.ApiServer.start_api", run_mock)
    default_conf["telegram"]["enabled"] = False
    rpc_manager = RPCManager(get_patched_chaindriftbot(mocker, default_conf))
