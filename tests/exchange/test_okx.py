...
from chaindrift.enums import CandleType, MarginMode, TradingMode
from chaindrift.exceptions import RetryableOrderError, TemporaryError
from chaindrift.exchange.common import API_RETRY_COUNT
from chaindrift.exchange.exchange import timeframe_to_minutes
...
def test_load_leverage_tiers_okx(default_conf, mocker, markets, tmp_path, caplog, time_machine):
    ...
    # fd_mock = mocker.patch('chaindrift.exchange.exchange.file_dump_json')
    api_mock = MagicMock()
...
def test_fetch_stoploss_order_okx(default_conf, mocker):
    default_conf["dry_run"] = False
    mocker.patch("chaindrift.exchange.common.time.sleep")
    api_mock = MagicMock()
