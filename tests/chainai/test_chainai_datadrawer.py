...
def test_update_historic_data(mocker, chainai_conf):
    ...
    historic_candles = len(chainai.dd.historic_data["ADA/BTC"]["5m"])
    ...
    chainai.dd.update_historic_data(strategy, chainai.dk)

    updated_historic_candles = len(chainai.dd.historic_data["ADA/BTC"]["5m"])

    assert updated_historic_candles - historic_candles == candle_difference
    shutil.rmtree(Path(chainai.dk.full_path))


def test_load_all_pairs_histories(mocker, chainai_conf):
    strategy = get_patched_chainai_strategy(mocker, chainai_conf)
    exchange = get_patched_exchange(mocker, chainai_conf)
    strategy.dp = DataProvider(chainai_conf, exchange)
    chainai = strategy.chainai
    ...
    assert len(chainai.dd.historic_data.keys()) == len(
        chainai_conf.get("exchange", {}).get("pair_whitelist")
    )
    assert len(chainai.dd.historic_data["ADA/BTC"]) == len(
        chainai_conf.get("chainai", {}).get("feature_parameters", {}).get("include_timeframes")
    )
    shutil.rmtree(Path(chainai.dk.full_path))


def test_get_base_and_corr_dataframes(mocker, chainai_conf):
    strategy = get_patched_chainai_strategy(mocker, chainai_conf)
    exchange = get_patched_exchange(mocker, chainai_conf)
    strategy.dp = DataProvider(chainai_conf, exchange)
    chainai = strategy.chainai
    ...
    assert len(corr_df.keys()) == len(
        chainai_conf.get("chainai", {}).get("feature_parameters", {}).get("include_corr_pairlist")
    )
...
def test_use_strategy_to_populate_indicators(mocker, chainai_conf):
    strategy = get_patched_chainai_strategy(mocker, chainai_conf)
    exchange = get_patched_exchange(mocker, chainai_conf)
    strategy.dp = DataProvider(chainai_conf, exchange)
...
def test_get_timerange_from_live_historic_predictions(mocker, chainai_conf):
    strategy = get_patched_chainai_strategy(mocker, chainai_conf)
    exchange = get_patched_exchange(mocker, chainai_conf)
    strategy.dp = DataProvider(chainai_conf, exchange)
    chainai = strategy.chainai
    chainai.live = False
    chainai.dk = ChainaiDataKitchen(chainai_conf)
    chainai.dk.live = False
    timerange = TimeRange.parse_timerange("20180126-20180130")
    chainai.dd.load_all_pair_histories(timerange, chainai.dk)
    sub_timerange = TimeRange.parse_timerange("20180128-20180130")
    _, base_df = chainai.dd.get_base_and_corr_dataframes(sub_timerange, "ADA/BTC", chainai.dk)
    base_df["5m"]["date_pred"] = base_df["5m"]["date"]
    chainai.dd.historic_predictions = {}
    chainai.dd.historic_predictions["ADA/USDT"] = base_df["5m"]
    chainai.dd.save_historic_predictions_to_disk()
    chainai.dd.save_global_metadata_to_disk({"start_dry_live_date": 1516406400})

    timerange = chainai.dd.get_timerange_from_live_historic_predictions()
    assert timerange.startts == 1516406400
    assert timerange.stopts == 1517356500


def test_get_timerange_from_backtesting_live_df_pred_not_found(mocker, chainai_conf):
    strategy = get_patched_chainai_strategy(mocker, chainai_conf)
    exchange = get_patched_exchange(mocker, chainai_conf)
    strategy.dp = DataProvider(chainai_conf, exchange)
    chainai = strategy.chainai
    with pytest.raises(OperationalException, match=r"Historic predictions not found.*"):
        chainai.dd.get_timerange_from_live_historic_predictions()


def test_set_initial_return_values(mocker, chainai_conf):
    """
    ...
    chainai.dd.historic_predictions[pair] = pd.DataFrame(historic_data)
    ...
    with patch("logging.Logger.warning") as mock_logger_warning:
        chainai.dd.set_initial_return_values(pair, new_pred_df, dataframe)

    # Assertions
    hist_pred_df = chainai.dd.historic_predictions[pair]
    model_return_df = chainai.dd.model_return_values[pair]
...
def test_set_initial_return_values_warning(mocker, chainai_conf):
    """
    Simple test of set_initial_return_values that hits the warning
    associated with leaving a ChainAI bot offline so long that the
