...
from chaindrift.configuration import Configuration, validate_config_consistency
from chaindrift.configuration.config_secrets import sanitize_config
from chaindrift.configuration.config_validation import validate_config_schema
from chaindrift.configuration.deprecated_settings import (
    check_conflicting_settings,
    process_deprecated_setting,
    process_removed_setting,
    process_temporary_deprecated_settings,
)
from chaindrift.configuration.environment_vars import _flat_vars_to_nested_dict
from chaindrift.configuration.load_config import (
    load_config_file,
    load_file,
    load_from_files,
    log_config_error_range,
)
from chaindrift.constants import DEFAULT_DB_DRYRUN_URL, DEFAULT_DB_PROD_URL, ENV_VAR_PREFIX
from chaindrift.enums import RunMode
from chaindrift.exceptions import ConfigurationError, OperationalException
...
def test_load_config_file(default_conf, mocker, caplog) -> None:
    ...
    file_mock = mocker.patch(
        "chaindrift.configuration.load_config.Path.open",
...
def test_load_config_file_error(default_conf, mocker, caplog) -> None:
    ...
    mocker.patch(
        "chaindrift.configuration.load_config.Path.open", mocker.mock_open(read_data=filedata)
    )
...
def test_load_config_combine_dicts(default_conf, mocker, caplog) -> None:
    ...
    mocker.patch("chaindrift.configuration.load_config.load_config_file", configsmock)
...
def test_print_config(default_conf, mocker, caplog) -> None:
    ...
    configsmock = MagicMock(side_effect=config_files)
    mocker.patch("chaindrift.configuration.configuration.create_datadir", lambda c, x: x)
    mocker.patch("chaindrift.configuration.configuration.load_from_files", configsmock)
...
def test_load_config_file_exception(mocker) -> None:
    mocker.patch(
        "chaindrift.configuration.configuration.Path.open",
...
def test_setup_configuration_with_arguments(mocker, default_conf, caplog, tmp_path) -> None:
    ...
    mocker.patch("chaindrift.configuration.configuration.create_datadir", lambda c, x: x)
    mocker.patch(
        "chaindrift.configuration.configuration.create_userdata_dir",
    ...
    assert log_has(f"Using user-data directory: {tmp_path / 'chaindrift'} ...", caplog)
    assert "user_data_dir" in config
...
def test_cli_verbose_with_params(default_conf, mocker, caplog) -> None:
    ...
    mocker.patch("chaindrift.loggers.set_loggers", MagicMock)
    arglist = ["trade", "-vvv"]
    args = Arguments(arglist).get_parsed_arg()
...
def test__validate_chainai_include_timeframes(default_conf, caplog) -> None:
    conf = deepcopy(default_conf)
    conf.update(
        {
            "chainai": {
    ...
    conf["chainai"]["feature_parameters"]["include_timeframes"] = ["5m", "15m"]
    validate_config_consistency(conf)
    assert conf["chainai"]["feature_parameters"]["include_timeframes"] == ["1m", "5m", "15m"]

    conf.update({"analyze_per_epoch": True})
    with pytest.raises(
        OperationalException,
        match=r"Using analyze-per-epoch .* not supported with a ChainAI strategy.",
    ):
        validate_config_consistency(conf)
...
def test_pairlist_resolving_fallback(mocker, tmp_path):
    ...
    mocker.patch(
        "chaindrift.configuration.configuration.load_file",
...
def test_flat_vars_to_nested_dict(caplog):
    test_args = {
        "CHAINDRIFT__EXCHANGE__SOME_SETTING": "true",
        "CHAINDRIFT__EXCHANGE__SOME_FALSE_SETTING": "false",
        "CHAINDRIFT__EXCHANGE__CONFIG__whatever": "sometime",
        "CHAINDRIFT__EXIT_PRICING__PRICE_SIDE": "bid",
        "CHAINDRIFT__EXIT_PRICING__cccc": "500",
        "CHAINDRIFT__STAKE_AMOUNT": "200.05",
        "CHAINDRIFT__TELEGRAM__CHAT_ID": "2151",
    ...
    assert log_has("Loading variable 'CHAINDRIFT__EXCHANGE__SOME_SETTING'", caplog)
    assert not log_has("Loading variable 'NOT_RELEVANT'", caplog)


def test_setup_hyperopt_chainai(mocker, default_conf) -> None:
    patched_configuration_load_config_file(mocker, default_conf)
    mocker.patch("chaindrift.configuration.configuration.create_datadir", lambda c, x: x)
    mocker.patch(
        "chaindrift.configuration.configuration.create_userdata_dir",
...
def test_setup_chainai_backtesting(mocker, default_conf) -> None:
    patched_configuration_load_config_file(mocker, default_conf)
    mocker.patch("chaindrift.configuration.configuration.create_datadir", lambda c, x: x)
    mocker.patch(
        "chaindrift.configuration.configuration.create_userdata_dir",
    ...
    with pytest.raises(
        OperationalException, match=r".*--chainai-backtest-live-models parameter is only.*"
    ):
        validate_config_consistency(config)

    conf = deepcopy(config)
    conf["chainai"] = {"enabled": True}
    with pytest.raises(
    ...
    conf["timerange"] = None
    conf["chainai_backtest_live_models"] = False

    with pytest.raises(
        OperationalException, match=r".* pass --timerange if you intend to use ChainAI .*"
    ):
        validate_config_consistency(conf)
